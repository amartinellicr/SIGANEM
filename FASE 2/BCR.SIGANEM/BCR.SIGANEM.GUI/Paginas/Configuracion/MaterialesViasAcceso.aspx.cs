...
public partial class MaterialesViasAcceso : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesViasAccesoEntidad materialesViasAccesoEntidad = new ConsultasWS.MaterialesViasAccesoEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Via_Acceso";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Via_Acceso";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCalleMaterialVias" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCalleMaterialVias", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodCalleMaterialVias", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesCalleMaterialVias", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Via_Acceso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Via_Acceso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCalleMaterialVias");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesViasAccesoEntidad.IdCalleMaterialVias = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesViasAccesoEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesViasAccesoEntidad entidad)
    {
        return wsConsultas.MaterialesViasAccesoEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesViasAccesoTotalFilas(entidad);
    }

    private List<MaterialesViasAccesoEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesViasAccesoConsultar(entidad).ToList();
    }
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            decimal resultadoFilas = (filasContador / maxFilas), contador = Math.Ceiling(resultadoFilas);

            decimal totalResultado = 0;
            if (resultadoFilas > contador)
                totalResultado = (resultadoFilas - contador);
            else if (resultadoFilas < contador)
                totalResultado = (contador - resultadoFilas);
            else if (resultadoFilas == contador)
                totalResultado = (int)contador;
    ...
    private void BindGridView(GridView gridView, List<MaterialesViasAccesoEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            materialesViasAccesoEntidad = null;
