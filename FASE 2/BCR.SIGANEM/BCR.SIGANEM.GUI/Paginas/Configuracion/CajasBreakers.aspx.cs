...
public partial class CajasBreakers : System.Web.UI.Page
{
    ...
    private ConsultasWS.CajasBreakersEntidad cajasBreakersEntidad = new ConsultasWS.CajasBreakersEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Caja_Breaker";
                    consultaEntidad.ColumnaOrdenar = "Cod_Caja_Breaker";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCajaBreaker" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCajaBreaker", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCajaBreaker", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCajaBreaker", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Caja_Breaker"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Caja_Breaker"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCajaBreaker");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cajasBreakersEntidad.IdCajaBreaker = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cajasBreakersEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CajasBreakersEntidad entidad)
    {
        return wsConsultas.CajasBreakersEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CajasBreakersTotalFilas(entidad);
    }

    private List<CajasBreakersEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CajasBreakersConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CajasBreakersEntidad> lista)
    {
