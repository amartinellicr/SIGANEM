...
public partial class Escaleras : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.EscalerasEntidad escalerasEntidad = new ConsultasWS.EscalerasEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Escalera";
					consultaEntidad.ColumnaOrdenar = "Cod_Escalera";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEscalera" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEscalera", lblID);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEscalera", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEscalera", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Escalera"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Escalera"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEscalera");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						escalerasEntidad.IdEscalera = int.Parse(lbl.Text);

						#endregion

                         resultado = this.Eliminar(escalerasEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EscalerasEntidad  entidad)
	{
		return wsConsultas.EscalerasEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
	{
		return wsConsultas.EscalerasTotalFilas( entidad);
	}

	private List<EscalerasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
	{
		return wsConsultas.EscalerasConsultar( entidad).ToList();
	}
    ...
    private void BindGridView(GridView  gridView, List<EscalerasEntidad>  lista)
    {
