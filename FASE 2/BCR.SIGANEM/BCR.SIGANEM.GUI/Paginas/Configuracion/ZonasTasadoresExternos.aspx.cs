...
public partial class ZonasTasadoresExternos : System.Web.UI.Page
{
    ...
    private ConsultasWS.ZonasTasadoresEntidad zonasTasadoresEntidad = new ConsultasWS.ZonasTasadoresEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Zona_Tasador";
                    consultaEntidad.ColumnaOrdenar = "Cod_Zona_Tasador";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdZonaTasador" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdZonaTasador", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodZonaTasador", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesZonaTasador", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Zona_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Zona_Tasador"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdZonaTasador");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        zonasTasadoresEntidad.IdZonaTasador = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(zonasTasadoresEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ZonasTasadoresEntidad _entidad)
    {
        _entidad.CodTipoZonaTasador = "X";
        return wsConsultas.ZonasTasadoresEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.ZonasTasadoresTotalFilas(_entidad, "X");
    }

    private List<ZonasTasadoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.ZonasTasadoresConsultar(_entidad, "X").ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<ZonasTasadoresEntidad> _lista)
    {
