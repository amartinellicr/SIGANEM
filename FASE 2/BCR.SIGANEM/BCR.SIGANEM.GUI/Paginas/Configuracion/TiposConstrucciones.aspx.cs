...
public partial class TiposConstrucciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposConstruccionesEntidad tiposConstruccionesEntidad = new ConsultasWS.TiposConstruccionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Construccion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Construccion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoConstruccion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoConstruccion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoConstruccion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoConstruccion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Construccion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Construccion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoConstruccion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposConstruccionesEntidad.IdTipoConstruccion = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposConstruccionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposConstruccionesEntidad _entidad)
    {
        return wsConsultas.TiposConstruccionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposConstruccionesTotalFilas(_entidad);
    }

    private List<TiposConstruccionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposConstruccionesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposConstruccionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            tiposConstruccionesEntidad = null;
