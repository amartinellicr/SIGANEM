...
public partial class NumerosLineas : System.Web.UI.Page
{
    ...
    private ConsultasWS.NumerosLineasEntidad numerosLineasEntidad = new ConsultasWS.NumerosLineasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Numero_Linea";
                    consultaEntidad.ColumnaOrdenar = "Numero_Linea";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdNumeroLinea" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdNumeroLinea", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("NumeroLinea", string.Empty, "Cantidad", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Cantidad", "Numero_Linea"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdNumeroLinea");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        numerosLineasEntidad.IdNumeroLinea = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(numerosLineasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(NumerosLineasEntidad _entidad)
    {
        return wsConsultas.NumerosLineasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.NumerosLineasTotalFilas(_entidad);
    }

    private List<NumerosLineasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.NumerosLineasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<NumerosLineasEntidad> _lista)
    {
