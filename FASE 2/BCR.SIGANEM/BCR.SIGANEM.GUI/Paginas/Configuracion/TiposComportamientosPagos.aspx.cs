...
public partial class TiposComportamientosPagos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposComportamientosPagosEntidad tiposComportamientosPagosEntidad = new ConsultasWS.TiposComportamientosPagosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Comportamiento_Pago";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Comportamiento_Pago";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoComportamientoPago" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoComportamientoPago", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoComportamientoPago", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoComportamientoPago", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Comportamiento_Pago"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Comportamiento_Pago"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoComportamientoPago");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposComportamientosPagosEntidad.IdTipoComportamientoPago = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposComportamientosPagosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposComportamientosPagosEntidad _entidad)
    {
        return wsConsultas.TiposComportamientosPagosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposComportamientosPagosTotalFilas(_entidad);
    }

    private List<TiposComportamientosPagosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposComportamientosPagosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposComportamientosPagosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            tiposComportamientosPagosEntidad = null;
