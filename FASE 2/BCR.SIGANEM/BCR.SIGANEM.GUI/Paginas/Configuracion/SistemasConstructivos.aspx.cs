...
public partial class SistemasConstructivos : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.SistemasConstructivosEntidad sistemasConstructivosEntidad = new ConsultasWS.SistemasConstructivosEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Sistema_Constructivo";
					consultaEntidad.ColumnaOrdenar = "Cod_Sistema_Constructivo";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdSistemaConstructivo" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdSistemaConstructivo", lblID);

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodSistemaConstructivo", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesSistemaConstructivo", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

    private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Sistema_Constructivo"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Sistema_Constructivo"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdSistemaConstructivo");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						sistemasConstructivosEntidad.IdSistemaConstructivo = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(sistemasConstructivosEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(SistemasConstructivosEntidad entidad)
	{
		return wsConsultas.SistemasConstructivosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.SistemasConstructivosTotalFilas(entidad);
	}

	private List<SistemasConstructivosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.SistemasConstructivosConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<SistemasConstructivosEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            sistemasConstructivosEntidad = null;
