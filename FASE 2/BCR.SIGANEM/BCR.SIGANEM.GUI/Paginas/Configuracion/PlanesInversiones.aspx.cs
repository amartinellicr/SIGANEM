...
public partial class PlanesInversiones : System.Web.UI.Page
{
    ...
    private ConsultasWS.PlanesInversionesEntidad _PlanesInversionesEntidad = new ConsultasWS.PlanesInversionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _ConsultaEntidad.ColumnaFiltro = "Cod_Plan_Inversion";
                    _ConsultaEntidad.ColumnaOrdenar = "Cod_Plan_Inversion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdPlanInversion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdPlanInversion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodPlanInversion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesPlanInversion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Plan_Inversion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Plan_Inversion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdPlanInversion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _PlanesInversionesEntidad.IdPlanInversion = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_PlanesInversionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(PlanesInversionesEntidad _entidad)
    {
        return wsConsultas.PlanesInversionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.PlanesInversionesTotalFilas(_entidad);
    }

    private List<PlanesInversionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.PlanesInversionesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<PlanesInversionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            _PlanesInversionesEntidad = null;
