...
public partial class Activos : System.Web.UI.Page
{
    ...
    private ConsultasWS.ActivosEntidad activosEntidad = new ConsultasWS.ActivosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Activo";
                    consultaEntidad.ColumnaOrdenar = "Cod_Activo";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdActivo" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdActivo", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodActivo", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesActivo", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Activo"));
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdActivo");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked == true)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        activosEntidad.IdActivo = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(activosEntidad);
                    }
                }
                ...
                int valorError = _result.ValorError;
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ActivosEntidad entidad)
    {
        return wsConsultas.ActivosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.ActivosTotalFilas(entidad);
    }

    private List<ActivosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.ActivosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<ActivosEntidad> lista)
    {
