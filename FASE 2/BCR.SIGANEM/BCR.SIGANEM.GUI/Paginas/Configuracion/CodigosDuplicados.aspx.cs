...
public partial class CodigosDuplicados : System.Web.UI.Page
{
    ...
    private ConsultasWS.CodigosDuplicadosEntidad codigosDuplicadosEntidad = new ConsultasWS.CodigosDuplicadosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Codigo_Duplicado";
                    consultaEntidad.ColumnaOrdenar = "Cod_Codigo_Duplicado";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCodigoDuplicado" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCodigoDuplicado", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCodigoDuplicado", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCodigoDuplicado", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDLLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Codigo_Duplicado"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Codigo_Duplicado"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCodigoDuplicado");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        codigosDuplicadosEntidad.IdCodigoDuplicado = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(codigosDuplicadosEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CodigosDuplicadosEntidad entidad)
    {
        return wsConsultas.CodigosDuplicadosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CodigosDuplicadosTotalFilas(entidad);
    }

    private List<CodigosDuplicadosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CodigosDuplicadosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CodigosDuplicadosEntidad> lista)
    {
