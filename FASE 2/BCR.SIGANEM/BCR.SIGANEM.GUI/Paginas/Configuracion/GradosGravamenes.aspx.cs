...
public partial class GradosGravamenes : System.Web.UI.Page
{
    ...
    private ConsultasWS.GradosGravamenesEntidad gradosGravamenesEntidad = new ConsultasWS.GradosGravamenesEntidad(); 
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    #region ENTIDAD CONSULTA
                     ...
                     consultaEntidad.ColumnaFiltro = "Cod_Grado_Gravamen";
                     consultaEntidad.ColumnaOrdenar = "Cod_Grado_Gravamen";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdGradoGravamen" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        TemplateField lblID = new TemplateField();
         gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGradoGravamen", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodGradoGravamen", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesGradoGravamen", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Grado_Gravamen"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Grado_Gravamen"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGradoGravamen");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        gradosGravamenesEntidad.IdGradoGravamen = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(gradosGravamenesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(GradosGravamenesEntidad  entidad)
    {
        return wsConsultas.GradosGravamenesEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
    {
        return wsConsultas.GradosGravamenesTotalFilas( entidad);
    }

    private List<GradosGravamenesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
    {
        return wsConsultas.GradosGravamenesConsultar( entidad).ToList();
    }
    ...
    private void BindGridView(GridView  gridView, List<GradosGravamenesEntidad>  lista)
    {
