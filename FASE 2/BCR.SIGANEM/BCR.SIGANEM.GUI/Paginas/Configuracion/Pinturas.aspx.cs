...
public partial class Pinturas : System.Web.UI.Page
{
    ...
    private ConsultasWS.PinturasEntidad pinturasEntidad = new ConsultasWS.PinturasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Pintura";
                    consultaEntidad.ColumnaOrdenar = "Cod_Pintura";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdPintura" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdPintura", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodPintura", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesPintura", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Pintura"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Pintura"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdPintura");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        pinturasEntidad.IdPintura = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(pinturasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(PinturasEntidad _entidad)
    {
        return wsConsultas.PinturasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.PinturasTotalFilas(_entidad);
    }

    private List<PinturasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.PinturasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<PinturasEntidad> _lista)
    {
