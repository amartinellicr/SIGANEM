...
public partial class Aplicables : System.Web.UI.Page
{
    ...
    private ConsultasWS.AplicablesEntidad aplicablesEntidad = new ConsultasWS.AplicablesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Aplicable";
                    consultaEntidad.ColumnaOrdenar = "Cod_Aplicable";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdAplicable" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdAplicable", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodAplicable", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesAplicable", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Aplicable"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Aplicable"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdAplicable");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        aplicablesEntidad.IdAplicable = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(aplicablesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(AplicablesEntidad entidad) 
    {
        return wsConsultas.AplicablesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.AplicablesTotalFilas(entidad);
    }

    private List<AplicablesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.AplicablesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<AplicablesEntidad> lista)
    {
