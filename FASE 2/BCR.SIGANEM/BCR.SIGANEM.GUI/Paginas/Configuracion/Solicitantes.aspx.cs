...
public partial class Solicitantes : System.Web.UI.Page
{
    ...
    private ConsultasWS.SolicitantesEntidad solicitantesEntidad = new ConsultasWS.SolicitantesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Solicitante";
                    consultaEntidad.ColumnaOrdenar = "Cod_Solicitante";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdSolicitante" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdSolicitante", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodSolicitante", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesSolicitante", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Solicitante"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Solicitante"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdSolicitante");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        solicitantesEntidad.IdSolicitante = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(solicitantesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(SolicitantesEntidad entidad)
    {
        return wsConsultas.SolicitantesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.SolicitantesTotalFilas(entidad);
    }

    private List<SolicitantesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.SolicitantesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<SolicitantesEntidad> lista)
    {
