...
public partial class Cantones : System.Web.UI.Page
{
    ...
    private ConsultasWS.CantonesEntidad cantonesEntidad = new ConsultasWS.CantonesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Canton";
                    consultaEntidad.ColumnaOrdenar = "Cod_Canton";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCanton" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCanton", lblID1);

        TemplateField lblID2 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdProvincia", lblID2);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCanton", string.Empty, "Código Cantón", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCanton", string.Empty, "Descripción Cantón", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodProvincia", string.Empty, "Código Provincia", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesProvincia", string.Empty, "Descripción Provincia", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código Cantón", "Cod_Canton"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción Cantón", "Des_Canton"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCanton");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cantonesEntidad.IdCanton = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cantonesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CantonesEntidad entidad)
    {
        return wsConsultas.CantonesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CantonesTotalFilas(entidad);
    }

    private List<CantonesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CantonesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CantonesEntidad> lista)
    {
