...
public partial class NivelesTerreno : System.Web.UI.Page
{
    ...
    private ConsultasWS.NivelesTerrenoEntidad nivelesTerrenoEntidad = new ConsultasWS.NivelesTerrenoEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Nivel_Terreno";
                    consultaEntidad.ColumnaOrdenar = "Cod_Nivel_Terreno";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdNivelTerreno" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdNivelTerreno", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodNivelTerreno", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesNivelTerreno", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Nivel_Terreno"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Nivel_Terreno"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdNivelTerreno");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        nivelesTerrenoEntidad.IdNivelTerreno = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(nivelesTerrenoEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(NivelesTerrenoEntidad  entidad)
    {
        return wsConsultas.NivelesTerrenosEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
    {
        return wsConsultas.NivelesTerrenosTotalFilas( entidad);
    }

    private List<NivelesTerrenoEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
    {
        return wsConsultas.NivelesTerrenosConsultar( entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<NivelesTerrenoEntidad> lista)
    {
