...
public partial class TiposGrados : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposGradosEntidad tiposGradosEntidad = new ConsultasWS.TiposGradosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Grado";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Grado";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoGrado" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoGrado", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoGrado", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoGrado", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Grado"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Grado"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoGrado");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposGradosEntidad.IdTipoGrado = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposGradosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposGradosEntidad _entidad)
    {
        return wsConsultas.TiposGradosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposGradosTotalFilas(_entidad);
    }

    private List<TiposGradosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposGradosConsultar(_entidad).ToList();
    }
    ...
    private bool AccesoPermisoPagina()
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                int permitido = wsSeguridad.UsuariosValidarAcceso(codUsuarioOculto.Value, pagina);
                if (permitido.Equals(0))
                {
    ...
    private void BindGridView(GridView _gridView, List<TiposGradosEntidad> _lista)
    {
