...
public partial class TiposMonedas : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposMonedasEntidad tiposMonedasEntidad = new ConsultasWS.TiposMonedasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Moneda";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Moneda";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoMoneda" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoMoneda", lblID);

        TemplateField lblID2 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdIndicadorMonedaExtranjera", lblID2);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoMoneda", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoMoneda", string.Empty, "Descripción", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodIndicadorMonedaExtranjera", string.Empty, "Cód Moneda Ext", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesIndicadorMonedaExtranjera", string.Empty, "Moneda Ext", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Capital", string.Empty, "Capital", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Pais", string.Empty, "País", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("FraccionMonetaria", string.Empty, "Fracción Mon", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Moneda"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Moneda"));
        this.ddlFiltro.Items.Add(new ListItem("Capital", "Des_Capital"));
        this.ddlFiltro.Items.Add(new ListItem("País", "Des_Pais"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Moneda"));
        this.ddlFiltro.Items.Add(new ListItem("Fracción Mon", "Fraccion_Monetaria"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoMoneda");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposMonedasEntidad.IdTipoMoneda = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposMonedasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposMonedasEntidad _entidad)
    {
        return wsConsultas.TiposMonedasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposMonedasTotalFilas(_entidad);
    }

    private List<TiposMonedasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposMonedasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposMonedasEntidad> _lista)
    {
