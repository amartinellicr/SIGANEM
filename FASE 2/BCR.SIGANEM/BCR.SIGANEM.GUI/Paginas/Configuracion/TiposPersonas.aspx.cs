...
public partial class TiposPersonas : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposPersonasEntidad tiposPersonasEntidad = new ConsultasWS.TiposPersonasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Persona";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Persona";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoPersona" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoPersona", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoPersona", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoPersona", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Persona"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Persona"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoPersona");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposPersonasEntidad.IdTipoPersona = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposPersonasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposPersonasEntidad _entidad)
    {
        return wsConsultas.TiposPersonasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposPersonasTotalFilas(_entidad);
    }

    private List<TiposPersonasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposPersonasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposPersonasEntidad> _lista)
    {
