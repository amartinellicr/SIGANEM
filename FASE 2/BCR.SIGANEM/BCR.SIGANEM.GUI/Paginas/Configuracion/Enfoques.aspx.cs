...
public partial class Enfoques : System.Web.UI.Page
{
    ...
    private ConsultasWS.EnfoquesEntidad _enfoquesEntidad = new ConsultasWS.EnfoquesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Enfoque";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Enfoque";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEnfoque" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEnfoque", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEnfoque", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEnfoque", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Enfoque"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Enfoque"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEnfoque");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _enfoquesEntidad.IdEnfoque = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_enfoquesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EnfoquesEntidad _entidad)
    {
        return wsConsultas.EnfoquesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.EnfoquesTotalFilas(_entidad);
    }

    private List<EnfoquesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.EnfoquesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<EnfoquesEntidad> _lista)
    {
