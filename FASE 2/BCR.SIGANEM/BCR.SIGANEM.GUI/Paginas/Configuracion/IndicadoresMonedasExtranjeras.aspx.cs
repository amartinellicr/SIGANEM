...
public partial class IndicadoresMonedasExtranjeras : System.Web.UI.Page
{
    ...
    private ConsultasWS.IndicadoresMonedasExtranjerasEntidad indicadoresMonedasExtranjerasEntidad = new ConsultasWS.IndicadoresMonedasExtranjerasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Indicador_Moneda_Extranjera";
                    consultaEntidad.ColumnaOrdenar = "Cod_Indicador_Moneda_Extranjera";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdIndicadorMonedaExtranjera" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdIndicadorMonedaExtranjera", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodIndicadorMonedaExtranjera", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesIndicadorMonedaExtranjera", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Indicador_Moneda_Extranjera"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Indicador_Moneda_Extranjera"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdIndicadorMonedaExtranjera");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        indicadoresMonedasExtranjerasEntidad.IdIndicadorMonedaExtranjera = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(indicadoresMonedasExtranjerasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(IndicadoresMonedasExtranjerasEntidad entidad)
    {
        return wsConsultas.IndicadoresMonedasExtranjerasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.IndicadoresMonedasExtranjerasTotalFilas(entidad);
    }

    private List<IndicadoresMonedasExtranjerasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.IndicadoresMonedasExtranjerasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<IndicadoresMonedasExtranjerasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            indicadoresMonedasExtranjerasEntidad = null;
