...
public partial class TiposIndicadoresInscripciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposIndicadoresInscripcionesEntidad tiposIndicadoresInscripcionesEntidad = new ConsultasWS.TiposIndicadoresInscripcionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Indicador_Inscripcion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Indicador_Inscripcion";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoIndicadorInscripcion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoIndicadorInscripcion", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoIndicadorInscripcion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoIndicadorInscripcion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Indicador_Inscripcion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Indicador_Inscripcion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoIndicadorInscripcion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposIndicadoresInscripcionesEntidad.IdTipoIndicadorInscripcion = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposIndicadoresInscripcionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposIndicadoresInscripcionesEntidad _entidad)
    {
        return wsConsultas.TiposIndicadoresInscripcionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposIndicadoresInscripcionesTotalFilas(_entidad);
    }

    private List<TiposIndicadoresInscripcionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposIndicadoresInscripcionesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposIndicadoresInscripcionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            tiposIndicadoresInscripcionesEntidad = null;
