...
public partial class Orientaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.OrientacionesEntidad orientacionesEntidad = new ConsultasWS.OrientacionesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Orientacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Orientacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdOrientacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdOrientacion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodOrientacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesOrientacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Orientacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Orientacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdOrientacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        orientacionesEntidad.IdOrientacion = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(orientacionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(OrientacionesEntidad _entidad)
    {
        return wsConsultas.OrientacionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.OrientacionesTotalFilas(_entidad);
    }

    private List<OrientacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.OrientacionesConsultar(_entidad).ToList();
    }
    ...
    {
        bool resultado = true;
        string page = Request.Url.Segments[Request.Url.Segments.Length - 1].Replace(".aspx", "");
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                int permits = wsSeguridad.UsuariosValidarAcceso(codUsuarioOculto.Value, page);
                if (permits.Equals(0))
                {
    ...
    private void BindGridView(GridView _gridView, List<OrientacionesEntidad> _lista)
    {
