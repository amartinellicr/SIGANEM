...
public partial class MaterialesConstruccionesPredominantes : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesConstruccionesPredominantesEntidad materialesConstruccionesPredominantesEntidad = new ConsultasWS.MaterialesConstruccionesPredominantesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Construccion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Construccion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMaterialConstruccionPredominante" };
                this.SetDataKeys(gridView, DataKeysString);

                // BUSCA NOMBRE DE LA PANTALLA//CAMBIAR WSDATASET
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMaterialConstruccionPredominante", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodMaterialConstruccionPredominante", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesMaterialConstruccionPredominante", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Construccion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Construccion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMaterialConstruccionPredominante");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesConstruccionesPredominantesEntidad.IdMaterialConstruccionPredominante = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesConstruccionesPredominantesEntidad);
                    }
                ...
                int valorErr = resultado.ValorError;
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesConstruccionesPredominantesEntidad entidad)
    {
        return wsConsultas.MaterialesConstruccionesPredominantesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesConstruccionesPredominantesTotalFilas(entidad);
    }

    private List<MaterialesConstruccionesPredominantesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesConstruccionesPredominantesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MaterialesConstruccionesPredominantesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            materialesConstruccionesPredominantesEntidad = null;
