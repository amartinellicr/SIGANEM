...
public partial class TiposEmisores : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposEmisoresEntidad tiposEmisoresEntidad = new ConsultasWS.TiposEmisoresEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Emisor";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Emisor";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoEmisor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoEmisor", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoEmisor", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoEmisor", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Emisor"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Emisor"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoEmisor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposEmisoresEntidad.IdTipoEmisor = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposEmisoresEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposEmisoresEntidad _entidad)
    {
        return wsConsultas.TiposEmisoresEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposEmisoresTotalFilas(_entidad);
    }

    private List<TiposEmisoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposEmisoresConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposEmisoresEntidad> _lista)
    {
