...
public partial class Entrepisos : System.Web.UI.Page
{
    ...
    private ConsultasWS.EntrepisosEntidad _entrepisosEntidad = new ConsultasWS.EntrepisosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Entrepiso";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Entrepiso";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEntrepiso" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEntrepiso", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEntrepiso", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEntrepiso", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLlFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Entrepiso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Entrepiso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEntrepiso");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _entrepisosEntidad.IdEntrepiso = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_entrepisosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EntrepisosEntidad _entidad)
    {
        return wsConsultas.EntrepisosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.EntrepisosTotalFilas(_entidad);
    }

    private List<EntrepisosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.EntrepisosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<EntrepisosEntidad> _lista)
    {
