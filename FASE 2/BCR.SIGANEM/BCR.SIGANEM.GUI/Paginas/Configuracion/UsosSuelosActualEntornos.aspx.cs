...
public partial class UsosSuelosActualEntornos : System.Web.UI.Page
{
    ...
    private ConsultasWS.UsosSuelosActualEntornosEntidad usosSuelosActualEntornosEntidad = new ConsultasWS.UsosSuelosActualEntornosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Uso_Suelo_Actual_Entorno";
                    consultaEntidad.ColumnaOrdenar = "Cod_Uso_Suelo_Actual_Entorno";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdUsoSueloActualEntorno" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdUsoSueloActualEntorno", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodUsoSueloActualEntorno", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesUsoSueloActualEntorno", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Uso_Suelo_Actual_Entorno"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Uso_Suelo_Actual_Entorno"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdUsoSueloActualEntorno");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        usosSuelosActualEntornosEntidad.IdUsoSueloActualEntorno = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(usosSuelosActualEntornosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(UsosSuelosActualEntornosEntidad _entidad)
    {
        return wsConsultas.UsosSuelosActualEntornosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.UsosSuelosActualEntornosTotalFilas(_entidad);
    }

    private List<UsosSuelosActualEntornosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.UsosSuelosActualEntornosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<UsosSuelosActualEntornosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            usosSuelosActualEntornosEntidad = null;
