...
public partial class EmisionesInstrumentos : System.Web.UI.Page
{
    ...
    private ConsultasWS.EmisionesInstrumentosEntidad emisionesInstrumentosEntidad = new ConsultasWS.EmisionesInstrumentosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Id_Emision_Instrumento";
                    consultaEntidad.ColumnaOrdenar = "Id_Emision_Instrumento";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEmisionInstrumento" };
                ...
                Button imgBtnNext = (Button)this.MasterPager1.FindControl("imgBtnNext");
                imgBtnNext.CommandName = "Next";
                imgBtnNext.Command += new CommandEventHandler(PagerCommand);
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEmisionInstrumento", lblID1);

        TemplateField lblID2 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEmisor", lblID2);

        TemplateField lblID3 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdInstrumento", lblID3);

        TemplateField lblID4 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMoneda", lblID4);

        TemplateField lblID5 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoClasificacionInstrumento", lblID5);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEmisor", string.Empty, "Código Emisor", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEmisor", string.Empty, "Descripción Emisor", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodInstrumento", string.Empty, "Código Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesInstrumento", string.Empty, "Descripción Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodMoneda", string.Empty, "Código Moneda", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesMoneda", string.Empty, "Descripción Moneda", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoClasificacionInstrumento", string.Empty, "Código Tipo Clasificación Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoClasificacionInstrumento", string.Empty, "Descripción Tipo Clasificación Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Serie", string.Empty, "Serie", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("ISIN", string.Empty, "Código ISIN", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Premio", string.Empty, "Monto Premio", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Vencimiento", string.Empty, "Fecha de Vencimiento", HorizontalAlign.Center, false, true));
    ...
    private void CargarDDLFiltro()
    {
        ...
        this.ddlFiltro.Items.Add(new ListItem("Serie", "Serie"));
        this.ddlFiltro.Items.Add(new ListItem("Código ISIN", "ISIN"));
        this.ddlFiltro.Items.Add(new ListItem("Monto Premio", "Premio"));
        this.ddlFiltro.Items.Add(new ListItem("Fecha de Vencimiento", "Vencimiento"));
    }
    ...
    protected void btnBuscar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                ...
                consultaEntidad.ColumnaOrdenar = "Serie";
    ...
    protected void btnModificar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEmisionInstrumento");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        emisionesInstrumentosEntidad.IdEmisionInstrumento = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(emisionesInstrumentosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EmisionesInstrumentosEntidad entidad)
    {
        return wsConsultas.EmisionesInstrumentosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.EmisionesInstrumentosTotalFilas(entidad);
    }

    private List<EmisionesInstrumentosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.EmisionesInstrumentosConsultar(entidad).ToList();
    }
    ...
    private bool AccesoPermisoPagina()
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                int permisos = wsSeguridad.UsuariosValidarAcceso(codUsuarioOculto.Value, pagina);
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            decimal resultadoFilas = (filasContador / maxFilas), rounded = Math.Ceiling(resultadoFilas);

            decimal totalResult = 0;
            if (resultadoFilas > rounded)
                totalResult = (resultadoFilas - rounded);
            else if (resultadoFilas < rounded)
                totalResult = (rounded - resultadoFilas);
            else if (resultadoFilas == rounded)
    ...
    protected Dictionary<string, string> SetRutaVentana()
    {
        ...
        datos.Add("idSesion", idSesionOculto.Value);
        datos.Add("codUsuario", codUsuarioOculto.Value);
        datos.Add("idPagina", pantallaIdOculto.Value);
        datos.Add("nombrePagina", pantallaNombreOculto.Value);
        datos.Add("moduloPagina", pantallaModuloOculto.Value);
        datos.Add("tituloPagina", pantallaTituloOculto.Value);
    ...
    protected ConsultasWS.BitacorasEntidad AsignarValoresBitacora(EnumTipoBitacora tipo)
    {
        try
        {
            #region ENTIDAD BITACORA

            bitacorasEntidad.CodAccion = bitacoraFlags.TipoBitacoraConsulta(tipo);
    ...
    private void BindGridView(GridView gridView, List<EmisionesInstrumentosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            if (eliminacion)
            ...
            emisionesInstrumentosEntidad = null;
