...
public partial class TiposServicios : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposServiciosEntidad tiposServiciosEntidad = new ConsultasWS.TiposServiciosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Servicio";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Servicio";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoServicio" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoServicio", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoServicio", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoServicio", string.Empty, "Descripción", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("TipoServicioTasador", string.Empty, "Tipo de Servicio", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Servicio"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Servicio"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo de Servicio", "Tipo_Servicio_Tasador"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoServicio");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposServiciosEntidad.IdTipoServicio = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposServiciosEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposServiciosEntidad _entidad)
    {
        return wsConsultas.TiposServiciosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposServiciosTotalFilas(_entidad);
    }

    private List<TiposServiciosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposServiciosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposServiciosEntidad> _lista)
    {
