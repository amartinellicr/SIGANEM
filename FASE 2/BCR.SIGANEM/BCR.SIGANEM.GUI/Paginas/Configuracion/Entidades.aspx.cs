...
public partial class Entidades : System.Web.UI.Page
{
    ...
    private ConsultasWS.EntidadesEntidad _entidadesEntidad = new ConsultasWS.EntidadesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Entidad";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Entidad";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEntidad" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEntidad", lblID1);

        TemplateField lblID2 = new TemplateField();
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdActivo", lblID2);

        TemplateField lblID3 = new TemplateField();
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdRegimenFiscalizacion", lblID3);

        TemplateField lblID4 = new TemplateField();
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoEntidad", lblID4);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEntidad", string.Empty, "Código de la Entidad", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEntidad", string.Empty, "Descripción de la Entidad", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodRegimenFiscalizacion", string.Empty, "Código Régimen Fiscalización", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesRegimenFiscalizacion", string.Empty, "Descripción Régimen Fiscalización", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoEntidad", string.Empty, "Código Tipo de Entidad", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoEntidad", string.Empty, "Descripción Tipo de Entidad", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesActivo", string.Empty, "Es Activo", HorizontalAlign.Center, false, true));
    ...
    private void CargarDDLFiltro()
    {
        ...
        this.ddlFiltro.Items.Add(new ListItem("Código de la Entidad", "Cod_Entidad"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción de la Entidad", "Des_Entidad"));
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEntidad");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _entidadesEntidad.IdEntidad = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_entidadesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EntidadesEntidad _entidad)
    {
        return wsConsultas.EntidadesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.EntidadesTotalFilas(_entidad);
    }

    private List<EntidadesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.EntidadesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<EntidadesEntidad> _lista)
    {
