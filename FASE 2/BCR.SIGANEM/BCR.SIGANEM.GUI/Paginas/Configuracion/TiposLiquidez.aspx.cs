...
public partial class TiposLiquidez : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposLiquidezEntidad tiposLiquidezEntidad = new ConsultasWS.TiposLiquidezEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Liquidez";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Liquidez";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoLiquidez" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoLiquidez", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoLiquidez", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoLiquidez", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Liquidez"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Liquidez"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoLiquidez");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposLiquidezEntidad.IdTipoLiquidez = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposLiquidezEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposLiquidezEntidad _entidad)
    {
        return wsConsultas.TiposLiquidezEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposLiquidezTotalFilas(_entidad);
    }

    private List<TiposLiquidezEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposLiquidezConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposLiquidezEntidad> _lista)
    {
