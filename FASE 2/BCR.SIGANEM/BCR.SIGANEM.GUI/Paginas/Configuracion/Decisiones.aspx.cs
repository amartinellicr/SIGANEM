...
public partial class Decisiones : System.Web.UI.Page
{
    ...
    private ConsultasWS.DecisionesEntidad decisionesEntidad = new ConsultasWS.DecisionesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Decision";
                    consultaEntidad.ColumnaOrdenar = "Cod_Decision";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdDecision" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdDecision", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodDecision", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesDecision", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Decision"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Decision"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdDecision");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        decisionesEntidad.IdDecision = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(decisionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(DecisionesEntidad entidad)
    {
        return wsConsultas.DecisionesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.DecisionesTotalFilas(entidad);
    }

    private List<DecisionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.DecisionesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<DecisionesEntidad> lista)
    {
