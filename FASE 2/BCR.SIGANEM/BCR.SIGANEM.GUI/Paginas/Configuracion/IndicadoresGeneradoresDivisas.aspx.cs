...
public partial class IndicadoresGeneradoresDivisas : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.IndicadoresGeneradoresDivisasEntidad indicadoresGeneradoresDivisasEntidad = new ConsultasWS.IndicadoresGeneradoresDivisasEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Indicador_Generador_Divisa";
					consultaEntidad.ColumnaOrdenar = "Cod_Indicador_Generador_Divisa";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdIndicadorGeneradorDivisa" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdIndicadorGeneradorDivisa", lblID);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodIndicadorGeneradorDivisa", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesIndicadorGeneradorDivisa", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Indicador_Generador_Divisa"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Indicador_Generador_Divisa"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdIndicadorGeneradorDivisa");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						indicadoresGeneradoresDivisasEntidad.IdIndicadorGeneradorDivisa = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(indicadoresGeneradoresDivisasEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(IndicadoresGeneradoresDivisasEntidad entidad)
	{
		return wsConsultas.IndicadoresGeneradoresDivisasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.IndicadoresGeneradoresDivisasTotalFilas(entidad);
	}

	private List<IndicadoresGeneradoresDivisasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.IndicadoresGeneradoresDivisasConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<IndicadoresGeneradoresDivisasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            indicadoresGeneradoresDivisasEntidad = null;
