...
public partial class Distritos : System.Web.UI.Page
{
    ...
    private GridViewColumn gridViewColumn = null;

    private Button btnBuscar = null;
    private Button bntBuscarCancelar = null;
    ...
    private ConsultasWS.DistritosEntidad distritosEntidad = new ConsultasWS.DistritosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Distrito";
                    consultaEntidad.ColumnaOrdenar = "Cod_Distrito";
                ...
                this.bntBuscarCancelar = (Button)this.MasterGrid1.FindControl("imgBtnClear");
                this.bntBuscarCancelar.Click += new EventHandler(btnBuscarCancelar_Click);
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdDistrito" };
                this.SetDataKeys(gridView, DataKeysString);

                // BUSCA NOMBRE DE LA PANTALLA//CAMBIAR SESION
                ...
                // ASIGNA EL TITULO AL MANTENIMIENTO//CAMBIAR SESION  
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdDistrito", lblID1);

        TemplateField lblID2 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCanton", lblID2);

        TemplateField lblID3 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdProvincia", lblID3);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodDistrito", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesDistrito", string.Empty, "Descripción", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodCanton", string.Empty, "Cód. Cantón", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesCanton", string.Empty, "Des. Cantón", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodProvincia", string.Empty, "Cód. Provincia", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesProvincia", string.Empty, "Des. Provincia", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Distrito"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Distrito"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdDistrito");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        distritosEntidad.IdDistrito = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(distritosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(DistritosEntidad entidad)
    {
        return wsConsultas.DistritosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.DistritosTotalFilas(entidad);
    }

    private List<DistritosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.DistritosConsultar(entidad).ToList();
    }
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            if (resultadoFilas > redondeo)
                totalResult = (resultadoFilas - redondeo);
            else if (resultadoFilas < redondeo)
                totalResult = (redondeo - resultadoFilas);
            else if (resultadoFilas == redondeo)
                totalResult = (int)redondeo;
    ...
    private void BindGridView(GridView gridView, List<DistritosEntidad> lista)
    {
