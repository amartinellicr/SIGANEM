...
public partial class TasadoresInternos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TasadoresEntidad tasadoresEntidad = new ConsultasWS.TasadoresEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tasador";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tasador";
    ...
    private void gridView_Init(object sender, EventArgs e) 
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTasador", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTasador", string.Empty, "Id Tasador", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesNombreTasador", string.Empty, "Nombre", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoPersona", string.Empty, "Tipo Persona", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoTasador", string.Empty, "Tipo Tasador", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Id", "Cod_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre", "Nombre_Tasador"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTasador");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tasadoresEntidad.IdTasador = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tasadoresEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TasadoresEntidad entidad)
    {
        return wsConsultas.TasadoresEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TasadoresTotalFilasInterno(entidad);
    }

    private List<TasadoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TasadoresConsultarInterno(entidad).ToList();
    }
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            decimal resultRows = (filasContador / maxFilas), redondeo = Math.Ceiling(resultRows);

            decimal totalResultado = 0;
            if (resultRows > redondeo)
                totalResultado = (resultRows - redondeo);
            else if (resultRows < redondeo)
                totalResultado = (redondeo - resultRows);
            else if (resultRows == redondeo)
    ...
    private void BindGridView(GridView gridView, List<TasadoresEntidad> lista)
    {
