...
public partial class ClasesGarantiasPrt17 : System.Web.UI.Page
{
    ...
    private ConsultasWS.ClasesGarantiasPrt17Entidad clasesGarantiasPrt17Entidad = new ConsultasWS.ClasesGarantiasPrt17Entidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Clase_Garantia_Prt_17";
                    consultaEntidad.ColumnaOrdenar = "Cod_Clase_Garantia_Prt_17";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdClaseGarantiaPrt17" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdClaseGarantiaPrt17", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodClaseGarantiaPrt17", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesClaseGarantiaPrt17", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Clase_Garantia_Prt_17"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Clase_Garantia_Prt_17"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdClaseGarantiaPrt17");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        clasesGarantiasPrt17Entidad.IdClaseGarantiaPrt17 = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(clasesGarantiasPrt17Entidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ClasesGarantiasPrt17Entidad entidad)
    {
        return wsConsultas.ClasesGarantiasPrt17Eliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.ClasesGarantiasPrt17TotalFilas(entidad);
    }

    private List<ClasesGarantiasPrt17Entidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.ClasesGarantiasPrt17Consultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<ClasesGarantiasPrt17Entidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            clasesGarantiasPrt17Entidad = null;
