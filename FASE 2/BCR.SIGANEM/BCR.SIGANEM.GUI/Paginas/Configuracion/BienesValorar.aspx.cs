...
public partial class BienesValorar : System.Web.UI.Page
{
    ...
    private ConsultasWS.BienesValorarEntidad bienesValorarEntidad = new ConsultasWS.BienesValorarEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Bien_Valorar";
                    consultaEntidad.ColumnaOrdenar = "Cod_Bien_Valorar";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdBienValorar" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdBienValorar", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodBienValorar", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesBienValorar", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Bien_Valorar"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Bien_Valorar"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdBienValorar");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        bienesValorarEntidad.IdBienValorar = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(bienesValorarEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(BienesValorarEntidad entidad)
    {
        return wsConsultas.BienesValorarEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.BienesValorarTotalFilas(entidad);
    }

    private List<BienesValorarEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.BienesValorarConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<BienesValorarEntidad> lista)
    {
