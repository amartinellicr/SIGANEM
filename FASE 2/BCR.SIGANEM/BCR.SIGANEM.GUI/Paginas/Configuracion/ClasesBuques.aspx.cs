...
public partial class ClasesBuques : System.Web.UI.Page
{
    ...
    private ConsultasWS.ClasesBuquesEntidad clasesBuquesEntidad = new ConsultasWS.ClasesBuquesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.IndiceInicioFila = (this.gridView.PageIndex * 10);
                    consultaEntidad.MaximoFilas = StaticParameters.RowCount;
                    consultaEntidad.ValorFiltro = String.Empty;
                    consultaEntidad.ColumnaFiltro = "Cod_Clase_Buque";
                    consultaEntidad.ColumnaOrdenar = "Cod_Clase_Buque";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdClaseBuque" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdClaseBuque", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodClaseBuque", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesClaseBuque", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Clase_Buque"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Clase_Buque"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdClaseBuque");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        clasesBuquesEntidad.IdClaseBuque = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(clasesBuquesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ClasesBuquesEntidad entidad)
    {
        return wsConsultas.ClasesBuquesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.ClasesBuquesTotalFilas(entidad);
    }

    private List<ClasesBuquesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.ClasesBuquesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<ClasesBuquesEntidad> lista)
    {
