...
public partial class Fiscalizadores : System.Web.UI.Page
{
    ...
    private ConsultasWS.FiscalizadoresEntidad fiscalizadoresEntidad = new ConsultasWS.FiscalizadoresEntidad(); 
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdFiscalizador" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdFiscalizador", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoPersona", string.Empty, "Tipo Persona Tasador", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTasador", string.Empty, "Id Tasador", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesNombreTasador", string.Empty, "Nombre Tasador", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoFiscalizador", string.Empty, "Tipo Fiscalizador", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Id Tasador", "Cod_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre Tasador", "Nombre_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Persona Tasador", "Des_Tipo_Persona"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Fiscalizador", "Des_Tipo_Fiscalizador"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdFiscalizador");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        fiscalizadoresEntidad.IdFiscalizador = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(fiscalizadoresEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(FiscalizadoresEntidad _entidad)
    {
        return wsConsultas.FiscalizadoresEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.FiscalizadoresTotalFilas(_entidad);
    }

    private List<FiscalizadoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.FiscalizadoresConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<FiscalizadoresEntidad> _lista)
    {
