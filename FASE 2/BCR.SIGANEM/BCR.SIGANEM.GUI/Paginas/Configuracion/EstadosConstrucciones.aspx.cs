...
public partial class EstadosConstrucciones : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.EstadosConstruccionesEntidad _estadosConstruccionesEntidad = new ConsultasWS.EstadosConstruccionesEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
			{
				...
				if (!Page.IsPostBack)
				{
					...
					_consultaEntidad.ColumnaFiltro = "Cod_Estado_Construccion";
					_consultaEntidad.ColumnaOrdenar = "Cod_Estado_Construccion";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEstadoConstruccion" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEstadoConstruccion", lblID);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEstadoConstruccion", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEstadoConstruccion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Estado_Construccion"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Estado_Construccion"));
	}
				...
				if (Page.IsPostBack)
				{
					if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEstadoConstruccion");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						_estadosConstruccionesEntidad.IdEstadoConstruccion = int.Parse(lbl.Text);

						#endregion

                        _result = this.Eliminar(_estadosConstruccionesEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EstadosConstruccionesEntidad _entidad)
	{
		return wsConsultas.EstadosConstruccionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
	{
		return wsConsultas.EstadosConstruccionesTotalFilas(_entidad);
	}

	private List<EstadosConstruccionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
	{
		return wsConsultas.EstadosConstruccionesConsultar(_entidad).ToList();
	}
    ...
    private void BindGridView(GridView _gridView, List<EstadosConstruccionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            _estadosConstruccionesEntidad = null;
