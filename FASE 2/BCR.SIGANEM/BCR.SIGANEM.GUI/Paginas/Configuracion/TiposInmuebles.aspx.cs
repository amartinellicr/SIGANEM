...
public partial class TiposInmuebles : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposInmueblesEntidad tiposInmueblesEntidad = new ConsultasWS.TiposInmueblesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Inmueble";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Inmueble";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoInmueble" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoInmueble", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoInmueble", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoInmueble", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Inmueble"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Inmueble"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoInmueble");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposInmueblesEntidad.IdTipoInmueble = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposInmueblesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposInmueblesEntidad _entidad)
    {
        return wsConsultas.TiposInmueblesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposInmueblesTotalFilas(_entidad);
    }

    private List<TiposInmueblesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposInmueblesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposInmueblesEntidad> _lista)
    {
