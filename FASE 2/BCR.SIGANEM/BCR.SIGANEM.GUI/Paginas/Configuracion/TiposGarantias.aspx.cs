...
public partial class TiposGarantias : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposGarantiasEntidad tiposGarantiasEntidad = new ConsultasWS.TiposGarantiasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Garantia";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Garantia";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoGarantia" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoGarantia", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoGarantia", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoGarantia", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Garantia"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Garantia"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoGarantia");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposGarantiasEntidad.IdTipoGarantia = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposGarantiasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposGarantiasEntidad _entidad)
    {
        return wsConsultas.TiposGarantiasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposGarantiasTotalFilas(_entidad);
    }

    private List<TiposGarantiasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposGarantiasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposGarantiasEntidad> _lista)
    {
