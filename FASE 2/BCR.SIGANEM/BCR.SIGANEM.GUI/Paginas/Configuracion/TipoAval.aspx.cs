...
public partial class TipoAval : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposAvalesEntidad tiposAvalesEntidad = new ConsultasWS.TiposAvalesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Aval";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Aval";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoAval" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoAval", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoAval", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoAval", string.Empty, "Descripción", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoPersona", string.Empty, "Tipo Persona Avalista", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("IdAvalista", string.Empty, "Id Avalista", HorizontalAlign.Center, false, true));
    ...
    private void CargarDDLFiltro()
    {
        ...
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Aval"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Aval"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Persona Avalista", "Des_Tipo_Persona"));
        this.ddlFiltro.Items.Add(new ListItem("Id Avalista", "Id_Avalista"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoAval");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposAvalesEntidad.IdTipoAval = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposAvalesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposAvalesEntidad _entidad)
    {
        return wsConsultas.TipoAvalEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TipoAvalTotalFilas(_entidad);
    }

    private List<TiposAvalesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TipoAvalConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposAvalesEntidad> _lista)
    {
