...
public partial class Emisores : System.Web.UI.Page
{
    ...
    private ConsultasWS.EmisoresEntidad emisoresEntidad = new ConsultasWS.EmisoresEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Emisor";
                    consultaEntidad.ColumnaOrdenar = "Cod_Emisor";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEmisor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEmisor", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEmisor", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEmisor", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Emisor"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Emisor"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEmisor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        emisoresEntidad.IdEmisor = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(emisoresEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EmisoresEntidad entidad)
    {
        return wsConsultas.EmisoresEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.EmisoresTotalFilas(entidad);
    }

    private List<EmisoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.EmisoresConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<EmisoresEntidad> lista)
    {
