...
public partial class Monedas : System.Web.UI.Page
{
    ...
    private ConsultasWS.MonedasEntidad monedasEntidad = new ConsultasWS.MonedasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Moneda";
                    consultaEntidad.ColumnaOrdenar = "Cod_Moneda";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMoneda" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMoneda", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodMoneda", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesMoneda", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Moneda"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Moneda"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMoneda");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        monedasEntidad.IdMoneda = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(monedasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MonedasEntidad  entidad)
    {
        return wsConsultas.MonedasEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
    {
        return wsConsultas.MonedasTotalFilas( entidad);
    }

    private List<MonedasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
    {
        return wsConsultas.MonedasConsultar( entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MonedasEntidad> lista)
    {
