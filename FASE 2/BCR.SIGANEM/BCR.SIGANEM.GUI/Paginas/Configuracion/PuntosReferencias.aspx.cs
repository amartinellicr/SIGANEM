...
public partial class PuntosReferencias : System.Web.UI.Page
{
    ...
    private ConsultasWS.PuntosReferenciasEntidad puntosReferenciasEntidad = new ConsultasWS.PuntosReferenciasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Punto_Referencia";
                    consultaEntidad.ColumnaOrdenar = "Cod_Punto_Referencia";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdPuntoReferencia" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdPuntoReferencia", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodPuntoReferencia", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesPuntoReferencia", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Punto_Referencia"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Punto_Referencia"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdPuntoReferencia");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        puntosReferenciasEntidad.IdPuntoReferencia = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(puntosReferenciasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(PuntosReferenciasEntidad _entidad)
    {
        return wsConsultas.PuntosReferenciasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.PuntosReferenciasTotalFilas(_entidad);
    }

    private List<PuntosReferenciasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.PuntosReferenciasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<PuntosReferenciasEntidad> _lista)
    {
