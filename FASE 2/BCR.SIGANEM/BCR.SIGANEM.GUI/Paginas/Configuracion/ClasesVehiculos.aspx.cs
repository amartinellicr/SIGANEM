...
public partial class ClasesVehiculos : System.Web.UI.Page
{
    ...
    private ConsultasWS.ClasesVehiculosEntidad _clasesVehiculosEntidad = new ConsultasWS.ClasesVehiculosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Clase_Vehiculo";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Clase_Vehiculo";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdClaseVehiculo" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdClaseVehiculo", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodClaseVehiculo", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesClaseVehiculo", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Clase_Vehiculo"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Clase_Vehiculo"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdClaseVehiculo");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _clasesVehiculosEntidad.IdClaseVehiculo = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_clasesVehiculosEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ClasesVehiculosEntidad _entidad)
    {
        return wsConsultas.ClasesVehiculosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.ClasesVehiculosTotalFilas(_entidad);
    }

    private List<ClasesVehiculosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.ClasesVehiculosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<ClasesVehiculosEntidad> _lista)
    {
