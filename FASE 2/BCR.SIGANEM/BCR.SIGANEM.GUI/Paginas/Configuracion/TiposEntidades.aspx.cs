...
public partial class TiposEntidades : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposEntidadesEntidad tiposEntidadesEntidad = new ConsultasWS.TiposEntidadesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Entidad";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Entidad";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTiposEntidades" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTiposEntidades", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTiposEntidades", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTiposEntidades", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Entidad"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Entidad"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTiposEntidades");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposEntidadesEntidad.IdTiposEntidades = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposEntidadesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposEntidadesEntidad _entidad)
    {
        return wsConsultas.TiposEntidadesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposEntidadesTotalFilas(_entidad);
    }

    private List<TiposEntidadesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposEntidadesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposEntidadesEntidad> _lista)
    {
