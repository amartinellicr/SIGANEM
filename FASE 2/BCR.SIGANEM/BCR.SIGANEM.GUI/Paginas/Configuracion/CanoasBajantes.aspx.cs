...
public partial class CanoasBajantes : System.Web.UI.Page
{
    ...
    private ConsultasWS.CanoasBajantesEntidad canoasBajantesEntidad = new ConsultasWS.CanoasBajantesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Canoa_Bajante";
                    consultaEntidad.ColumnaOrdenar = "Cod_Canoa_Bajante";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCanoaBajante" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCanoaBajante", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCanoaBajante", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCanoaBajante", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Canoa_Bajante"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Canoa_Bajante"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCanoaBajante");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        canoasBajantesEntidad.IdCanoaBajante = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(canoasBajantesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CanoasBajantesEntidad entidad)
    {
        return wsConsultas.CanoasBajantesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CanoasBajantesTotalFilas(entidad);
    }

    private List<CanoasBajantesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CanoasBajantesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<CanoasBajantesEntidad> lista)
    {
