...
public partial class EstructurasTechos : System.Web.UI.Page
{
    ...
    private ConsultasWS.EstructurasTechosEntidad estructurasTechosEntidad = new ConsultasWS.EstructurasTechosEntidad();
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    #region ENTIDAD CONSULTA
                     ...
                     consultaEntidad.ColumnaFiltro = "Cod_Estructura_Techo";
                     consultaEntidad.ColumnaOrdenar = "Cod_Estructura_Techo";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEstructuraTecho" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        TemplateField lblID = new TemplateField();
         gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEstructuraTecho", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEstructuraTecho", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEstructuraTecho", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Estructura_Techo"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Estructura_Techo"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEstructuraTecho");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        estructurasTechosEntidad.IdEstructuraTecho = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(estructurasTechosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EstructurasTechosEntidad  entidad)
    {
        return wsConsultas.EstructurasTechosEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
    {
        return wsConsultas.EstructurasTechosTotalFilas( entidad);
    }

    private List<EstructurasTechosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
    {
        return wsConsultas.EstructurasTechosConsultar( entidad).ToList();
    }
    ...
    private bool AccesoPermisoPagina()
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                int permisas = wsSeguridad.UsuariosValidarAcceso(codUsuarioOculto.Value, _page);
                if (permisas.Equals(0))
                {
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            if (resultadoFilas > rounded)
                totalResultado = (resultadoFilas - rounded);
            else if (resultadoFilas < rounded)
                totalResultado = (rounded - resultadoFilas);
            else if (resultadoFilas == rounded)
                totalResultado = (int)rounded;
    ...
    protected ConsultasWS.BitacorasEntidad AsignarValoresBitacora(EnumTipoBitacora  tipo)
    {
        try
        {
            #region ENTIDAD BITACORA
             ...
             bitacorasEntidad.CodAccion = _bitacoraFlags.TipoBitacoraConsulta( tipo);
    ...
    private void BindGridView(GridView  gridView, List<EstructurasTechosEntidad>  lista)
    {
