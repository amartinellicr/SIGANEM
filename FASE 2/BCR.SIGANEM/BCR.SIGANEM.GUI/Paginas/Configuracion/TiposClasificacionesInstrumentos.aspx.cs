...
public partial class TiposClasificacionesInstrumentos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposClasificacionesInstrumentosEntidad tiposClasificacionesInstrumentosEntidad = new ConsultasWS.TiposClasificacionesInstrumentosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Clasificacion_Instrumento";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Clasificacion_Instrumento";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoClasificacionInstrumento" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoClasificacionInstrumento", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoClasificacionInstrumento", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoClasificacionInstrumento", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Clasificacion_Instrumento"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Clasificacion_Instrumento"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoClasificacionInstrumento");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposClasificacionesInstrumentosEntidad.IdTipoClasificacionInstrumento = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposClasificacionesInstrumentosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposClasificacionesInstrumentosEntidad _entidad)
    {
        return wsConsultas.TiposClasificacionesInstrumentosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposClasificacionesInstrumentosTotalFilas(_entidad);
    }

    private List<TiposClasificacionesInstrumentosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposClasificacionesInstrumentosConsultar(_entidad).ToList();
    }
    ...
    protected void Set_RegistrosLabelIndex()
    {
        try
        {
            ...
            decimal rowCount = this.TotalFilas(consultaTotalFilas);
    ...
    private void BindGridView(GridView _gridView, List<TiposClasificacionesInstrumentosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            tiposClasificacionesInstrumentosEntidad = null;
