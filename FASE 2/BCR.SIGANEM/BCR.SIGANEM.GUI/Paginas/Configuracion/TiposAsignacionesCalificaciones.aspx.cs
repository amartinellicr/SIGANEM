...
public partial class TiposAsignacionesCalificaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposAsignacionesCalificacionesEntidad tiposAsignacionesCalificacionesEntidad = new ConsultasWS.TiposAsignacionesCalificacionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Asignacion_Calificacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Asignacion_Calificacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoAsignacionCalificacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoAsignacionCalificacion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoAsignacionCalificacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoAsignacionCalificacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Asignacion_Calificacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Asignacion_Calificacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoAsignacionCalificacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposAsignacionesCalificacionesEntidad.IdTipoAsignacionCalificacion = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposAsignacionesCalificacionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposAsignacionesCalificacionesEntidad entidad)
    {
        return wsConsultas.TiposAsignacionesCalificacionesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TiposAsignacionesCalificacionesTotalFilas(entidad);
    }

    private List<TiposAsignacionesCalificacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TiposAsignacionesCalificacionesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<TiposAsignacionesCalificacionesEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            tiposAsignacionesCalificacionesEntidad = null;
