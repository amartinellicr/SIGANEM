...
public partial class IndicacionesAjustesAreas : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.IndicacionesAjustesAreasEntidad _indicacionesAjustesAreasEntidad = new ConsultasWS.IndicacionesAjustesAreasEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Indicacion_Ajuste_Area";
					consultaEntidad.ColumnaOrdenar = "Cod_Indicacion_Ajuste_Area";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdIndicacionAjusteArea" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdIndicacionAjusteArea", lblID);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodIndicacionAjusteArea", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesIndicacionAjusteArea", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Indicacion_Ajuste_Area"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Indicacion_Ajuste_Area"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdIndicacionAjusteArea");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						_indicacionesAjustesAreasEntidad.IdIndicacionAjusteArea = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(_indicacionesAjustesAreasEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(IndicacionesAjustesAreasEntidad entidad)
	{
		return wsConsultas.IndicacionesAjustesAreasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.IndicacionesAjustesAreasTotalFilas(entidad);
	}

	private List<IndicacionesAjustesAreasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.IndicacionesAjustesAreasConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<IndicacionesAjustesAreasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool disponible)
    {
        if (!disponible)
        {
            ...
            _indicacionesAjustesAreasEntidad = null;
        }
    }
