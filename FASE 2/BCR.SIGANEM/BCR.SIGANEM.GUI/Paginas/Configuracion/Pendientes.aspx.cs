...
public partial class Pendientes : System.Web.UI.Page
{
    ...
    private ConsultasWS.PendientesEntidad pendientesEntidad = new ConsultasWS.PendientesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Pendiente";
                    consultaEntidad.ColumnaOrdenar = "Cod_Pendiente";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdPendiente" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdPendiente", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodPendiente", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesPendiente", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Pendiente"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Pendiente"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdPendiente");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        pendientesEntidad.IdPendiente = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(pendientesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(PendientesEntidad _entidad)
    {
        return wsConsultas.PendientesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.PendientesTotalFilas(_entidad);
    }

    private List<PendientesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.PendientesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<PendientesEntidad> _lista)
    {
