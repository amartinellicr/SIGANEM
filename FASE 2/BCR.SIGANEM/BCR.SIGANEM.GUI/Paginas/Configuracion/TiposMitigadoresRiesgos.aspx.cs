...
public partial class TiposMitigadoresRiesgos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposMitigadoresRiesgosEntidad tiposMitigadoresRiesgosEntidad = new ConsultasWS.TiposMitigadoresRiesgosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Mitigador_Riesgo";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Mitigador_Riesgo";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoMitigadorRiesgo" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoMitigadorRiesgo", lblID);

        //Req 1-24653624 
        //TemplateField lblID2 = new TemplateField();
        //_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCategoriaCalificacion", lblID2);

        //Req 1-24653624 
        //TemplateField lblID3 = new TemplateField();
        //_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoGarantia", lblID3);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoMitigadorRiesgo", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoMitigadorRiesgo", string.Empty, "Descripción", HorizontalAlign.Center, false, true));

        //Req 1-24653624 
        //this.gridViewColumn = new GridViewColumn();
        //this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("PorcAceptacionCalificacionRiesgo", string.Empty, "% Aceptación", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Mitigador_Riesgo"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Mitigador_Riesgo"));
        //Req 1-24653624 
        //this.ddlFiltro.Items.Add(new ListItem("% Aceptación", "Porc_Aceptacion_Calificacion_Riesgo"));        
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoMitigadorRiesgo");
    ...
    protected void btnEliminar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() > 0) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            CheckBox checkBoxColumn = (CheckBox)gridView.Rows[row.RowIndex].FindControl("chkBox1");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposMitigadoresRiesgosEntidad.IdTipoMitigadorRiesgo = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposMitigadoresRiesgosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposMitigadoresRiesgosEntidad _entidad)
    {
        return wsConsultas.TiposMitigadoresRiesgosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposMitigadoresRiesgosTotalFilas(_entidad);
    }

    private List<TiposMitigadoresRiesgosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposMitigadoresRiesgosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposMitigadoresRiesgosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            tiposMitigadoresRiesgosEntidad = null;
