...
public partial class TiposDocumentosLegales : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposDocumentosLegalesEntidad tiposDocumentosLegalesEntidad = new ConsultasWS.TiposDocumentosLegalesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Documento_Legal";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Documento_Legal";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoDocumentoLegal" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoDocumentoLegal", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoDocumentoLegal", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoDocumentoLegal", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Documento_Legal"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Documento_Legal"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoDocumentoLegal");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposDocumentosLegalesEntidad.IdTipoDocumentoLegal = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposDocumentosLegalesEntidad);
                    }
    ...
    {
        return wsConsultas.TiposDocumentosLegalesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposDocumentosLegalesTotalFilas(_entidad);
    }

    private List<TiposDocumentosLegalesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposDocumentosLegalesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposDocumentosLegalesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            tiposDocumentosLegalesEntidad = null;
