...
public partial class MaterialesParedesExternasInternas : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesExternosInternosEntidad materialesExternosInternosEntidad = new ConsultasWS.MaterialesExternosInternosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Externo_Interno";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Externo_Interno";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMaterialExternoInterno" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMaterialExternoInterno", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodMaterialExternoInterno", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesMaterialExternoInterno", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Externo_Interno"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Externo_Interno"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMaterialExternoInterno");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesExternosInternosEntidad.IdMaterialExternoInterno = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesExternosInternosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesExternosInternosEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasInternasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasInternasTotalFilas(entidad);
    }

    private List<MaterialesExternosInternosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasInternasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MaterialesExternosInternosEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            materialesExternosInternosEntidad = null;
