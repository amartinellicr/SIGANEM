...
public partial class TiposGruposFinancieros : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposGruposFinancierosEntidad tiposGruposFinancierosEntidad = new ConsultasWS.TiposGruposFinancierosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Grupo_Financiero";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Grupo_Financiero";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoGrupoFinanciero" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoGrupoFinanciero", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoGrupoFinanciero", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoGrupoFinanciero", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Grupo_Financiero"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Grupo_Financiero"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoGrupoFinanciero");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposGruposFinancierosEntidad.IdTipoGrupoFinanciero = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposGruposFinancierosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposGruposFinancierosEntidad _entidad)
    {
        return wsConsultas.TiposGruposFinancierosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposGruposFinancierosTotalFilas(_entidad);
    }

    private List<TiposGruposFinancierosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposGruposFinancierosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposGruposFinancierosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            tiposGruposFinancierosEntidad = null;
