...
public partial class PlazosCalificaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.PlazosCalificacionesEntidad plazosCalificacionesEntidad = new ConsultasWS.PlazosCalificacionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Plazo_Calificacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Plazo_Calificacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdPlazoCalificacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdPlazoCalificacion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodPlazoCalificacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesPlazoCalificacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Plazo_Calificacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Plazo_Calificacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdPlazoCalificacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        plazosCalificacionesEntidad.IdPlazoCalificacion = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(plazosCalificacionesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(PlazosCalificacionesEntidad _entidad)
    {
        return wsConsultas.PlazosCalificacionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.PlazosCalificacionesTotalFilas(_entidad);
    }

    private List<PlazosCalificacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.PlazosCalificacionesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<PlazosCalificacionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            plazosCalificacionesEntidad = null;
