...
public partial class TiposAvalesFianzas : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposAvalesFianzasEntidad tiposAvalesFianzasEntidad = new ConsultasWS.TiposAvalesFianzasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Aval_Fianza";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Aval_Fianza";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoAvalFianza" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoAvalFianza", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoAvalFianza", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoAvalFianza", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Aval_Fianza"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Aval_Fianza"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoAvalFianza");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposAvalesFianzasEntidad.IdTipoAvalFianza = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposAvalesFianzasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposAvalesFianzasEntidad entidad)
    {
        return wsConsultas.TiposAvalesFianzasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TiposAvalesFianzasTotalFilas(entidad);
    }

    private List<TiposAvalesFianzasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TiposAvalesFianzasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<TiposAvalesFianzasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            tiposAvalesFianzasEntidad = null;
