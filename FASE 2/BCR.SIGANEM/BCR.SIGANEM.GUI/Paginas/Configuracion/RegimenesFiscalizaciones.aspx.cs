...
public partial class RegimenesFiscalizaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.RegimenesFiscalizacionesEntidad regimenesFiscalizacionesEntidad = new ConsultasWS.RegimenesFiscalizacionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Regimen_Fiscalizacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Regimen_Fiscalizacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdRegimenFiscalizacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdRegimenFiscalizacion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodRegimenFiscalizacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesRegimenFiscalizacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Regimen_Fiscalizacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Regimen_Fiscalizacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdRegimenFiscalizacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        regimenesFiscalizacionesEntidad.IdRegimenFiscalizacion = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(regimenesFiscalizacionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(RegimenesFiscalizacionesEntidad _entidad)
    {
        return wsConsultas.RegimenesFiscalizacionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.RegimenesFiscalizacionesTotalFilas(_entidad);
    }

    private List<RegimenesFiscalizacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.RegimenesFiscalizacionesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<RegimenesFiscalizacionesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            regimenesFiscalizacionesEntidad = null;
