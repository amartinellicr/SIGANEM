...
public partial class LotesSegregados : System.Web.UI.Page
{
    ...
    private ConsultasWS.LotesSegregadosEntidad lotesSegregadosEntidad = new ConsultasWS.LotesSegregadosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cantidad_Lote_Segregado";
                    consultaEntidad.ColumnaOrdenar = "Cantidad_Lote_Segregado";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdLoteSegregado" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdLoteSegregado", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CantidadLotesSegregados", string.Empty, "Cantidad Lote Segregado", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Cantidad Lote Segregado", "Cantidad_Lote_Segregado"));
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdLoteSegregado");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        lotesSegregadosEntidad.IdLoteSegregado = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(lotesSegregadosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(LotesSegregadosEntidad entidad)
    {
        return wsConsultas.LotesSegregadosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.LotesSegregadosTotalFilas(entidad);
    }

    private List<LotesSegregadosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.LotesSegregadosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<LotesSegregadosEntidad> _lista)
    {
