...
public partial class CanalizacionesElectricas : System.Web.UI.Page
{
    ...
    private ConsultasWS.CanalizacionesElectricasEntidad canalizacionesElectricasEntidad = new ConsultasWS.CanalizacionesElectricasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Canalizacion_Electrica";
                    consultaEntidad.ColumnaOrdenar = "Cod_Canalizacion_Electrica";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCanalizacionElectrica" };
                ...
                Button btnAceptarConfirmar = (Button)this.InformarBox1.FindControl("wucBtnAceptar");
                btnAceptarConfirmar.Click += new EventHandler(btnAceptarInformar_Click);
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCanalizacionElectrica", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodCanalizacionElectrica", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesCanalizacionElectrica", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Canalizacion_Electrica"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Canalizacion_Electrica"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCanalizacionElectrica");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        canalizacionesElectricasEntidad.IdCanalizacionElectrica = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(canalizacionesElectricasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CanalizacionesElectricasEntidad entidad)
    {
        return wsConsultas.CanalizacionesElectricasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CanalizacionesElectricasTotalFilas(entidad);
    }

    private List<CanalizacionesElectricasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CanalizacionesElectricasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CanalizacionesElectricasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            canalizacionesElectricasEntidad = null;
