...
public partial class Unidades : System.Web.UI.Page
{
    ...
    private ConsultasWS.UnidadesEntidad unidadesEntidad = new ConsultasWS.UnidadesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Unidad";
                    consultaEntidad.ColumnaOrdenar = "Cod_Unidad";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdUnidad" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdUnidad", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodUnidad", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesUnidad", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Unidad"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Unidad"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdUnidad");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        unidadesEntidad.IdUnidad = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(unidadesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(UnidadesEntidad _entidad)
    {
        return wsConsultas.UnidadesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.UnidadesTotalFilas(_entidad);
    }

    private List<UnidadesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.UnidadesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<UnidadesEntidad> _lista)
    {
