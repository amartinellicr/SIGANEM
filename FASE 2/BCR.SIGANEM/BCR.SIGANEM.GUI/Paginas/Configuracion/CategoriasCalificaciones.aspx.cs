...
public partial class CategoriasCalificaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.CategoriasCalificacionesEntidad categoriasCalificacionesEntidad = new ConsultasWS.CategoriasCalificacionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Categoria_Calificacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Categoria_Calificacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCategoriaCalificacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCategoriaCalificacion", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCategoriaCalificacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCategoriaCalificacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Categoria_Calificacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Categoria_Calificacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCategoriaCalificacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        categoriasCalificacionesEntidad.IdCategoriaCalificacion = int.Parse(lbl.Text);

                        #endregion

                        result = this.Eliminar(categoriasCalificacionesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CategoriasCalificacionesEntidad entidad)
    {
        return wsConsultas.CategoriasCalificacionesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CategoriasCalificacionesTotalFilas(entidad);
    }

    private List<CategoriasCalificacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CategoriasCalificacionesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<CategoriasCalificacionesEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            categoriasCalificacionesEntidad = null;
