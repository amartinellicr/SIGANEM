...
public partial class Notarios : System.Web.UI.Page
{
    ...
    private ConsultasWS.NotariosEntidad notariosEntidad = new ConsultasWS.NotariosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Notario";
                    consultaEntidad.ColumnaOrdenar = "Cod_Notario";
                ...
                this.bntBuscarCancelar.Click += new EventHandler(bntBuscarCancelar_Click);
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdNotario", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoPersona", string.Empty, "Tipo Persona Notario", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodNotario", string.Empty, "Id Notario", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesNotario", string.Empty, "Nombre Notario", HorizontalAlign.Center, false, true));
        
        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoNotario", string.Empty, "Tipo Notario", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Id Notario", "Cod_Notario"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre Notario", "Des_Notario"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Persona Notario", "Des_Tipo_Persona"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Notario", "Des_Tipo_Notario"));
    }
    ...
    protected void bntBuscarCancelar_Click(object sender, EventArgs e)
    {
        try
        {
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdNotario");
    ...
    protected void btnAceptarEliminar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                ConsultasWS.RespuestaEntidad result = null;
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        notariosEntidad.IdNotario = int.Parse(lbl.Text);

                        #endregion

                        result = this.Eliminar(notariosEntidad);
                    }
                }
                #region CONFIRMACIÓN ELIMINAR

                this.mpeInformarBox.Show();
                int valor = result.ValorEstado;
                int valorErr = result.ValorError;
    ...
    protected virtual void PagerCommand(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                ...
                int maxFilas = StaticParameters.RowCount;
                int StartRowIndex = 0;
                ...
                switch (e.CommandName)
                {
                    case "Next":
                        ...
                        {
                            StartRowIndex = intSlide * maxFilas;
                            #region ENTIDAD CONSULTA

                            consultaEntidad.IndiceInicioFila = StartRowIndex;
                            consultaEntidad.MaximoFilas = maxFilas;
                            consultaEntidad.ValorFiltro = this.txtFiltro.Text;
                    ...
                    case "Previous":
                        ...
                        {
                            StartRowIndex = (intSlide - 2) * maxFilas;
                            #region ENTIDAD CONSULTA
                    ...
                    case "Last":
                        if (Int32.Parse(this.txtSlide.Text) <= this.Registros)
                        {
                            ...
                            StartRowIndex = (this.Registros - 1) * maxFilas;
                            #region ENTIDAD CONSULTA
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(NotariosEntidad entidad)
    {
        return wsConsultas.NotariosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.NotariosTotalFilas(entidad);
    }

    private List<NotariosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.NotariosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<NotariosEntidad> lista)
    {
