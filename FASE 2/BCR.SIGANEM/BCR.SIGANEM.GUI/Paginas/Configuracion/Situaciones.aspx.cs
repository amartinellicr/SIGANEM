...
public partial class Situaciones : System.Web.UI.Page
{
    ...
    private ConsultasWS.SituacionesEntidad situacionesEntidad = new ConsultasWS.SituacionesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Situacion";
                    consultaEntidad.ColumnaOrdenar = "Cod_Situacion";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdSituacion" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdSituacion", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodSituacion", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesSituacion", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Situacion"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Situacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdSituacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        situacionesEntidad.IdSituacion = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(situacionesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(SituacionesEntidad entidad)
    {
        return wsConsultas.SituacionesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.SituacionesTotalFilas(entidad);
    }

    private List<SituacionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.SituacionesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<SituacionesEntidad> lista)
    {
