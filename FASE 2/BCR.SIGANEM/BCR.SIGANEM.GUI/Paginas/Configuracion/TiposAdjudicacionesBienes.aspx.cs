...
public partial class TiposAdjudicacionesBienes : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposAdjudicacionesBienesEntidad tiposAdjudicacionesBienesEntidad = new ConsultasWS.TiposAdjudicacionesBienesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Adjudicacion_Bien";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Adjudicacion_Bien";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoAdjudicacionBien" };
                ...
                Button imgBtnLast = (Button)this.MasterPager1.FindControl("imgBtnLast");
                imgBtnLast.CommandName = "Last";
                imgBtnLast.Command += new CommandEventHandler(PagerCommand);
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoAdjudicacionBien", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoAdjudicacionBien", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoAdjudicacionBien", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Adjudicacion_Bien"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Adjudicacion_Bien"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoAdjudicacionBien");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposAdjudicacionesBienesEntidad.IdTipoAdjudicacionBien = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposAdjudicacionesBienesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposAdjudicacionesBienesEntidad entidad)
    {
        return wsConsultas.TiposAdjudicacionesBienesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TiposAdjudicacionesBienesTotalFilas(entidad);
    }

    private List<TiposAdjudicacionesBienesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TiposAdjudicacionesBienesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<TiposAdjudicacionesBienesEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            tiposAdjudicacionesBienesEntidad = null;
