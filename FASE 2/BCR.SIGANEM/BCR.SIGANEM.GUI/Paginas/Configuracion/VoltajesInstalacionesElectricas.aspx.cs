...
public partial class VoltajesInstalacionesElectricas : System.Web.UI.Page
{
    ...
    private ConsultasWS.VoltajesInstalacionesElectricasEntidad voltajesInstalacionesElectricasEntidad = new ConsultasWS.VoltajesInstalacionesElectricasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Voltaje_Instalacion_Electrica";
                    consultaEntidad.ColumnaOrdenar = "Cod_Voltaje_Instalacion_Electrica";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdVoltajeInstalacionElectrica" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdVoltajeInstalacionElectrica", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodVoltajeInstalacionElectrica", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesVoltajeInstalacionElectrica", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Voltaje_Instalacion_Electrica"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Voltaje_Instalacion_Electrica"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdVoltajeInstalacionElectrica");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        voltajesInstalacionesElectricasEntidad.IdVoltajeInstalacionElectrica = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(voltajesInstalacionesElectricasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(VoltajesInstalacionesElectricasEntidad _entidad)
    {
        return wsConsultas.VoltajesInstalacionesElectricasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.VoltajesInstalacionesElectricasTotalFilas(_entidad);
    }

    private List<VoltajesInstalacionesElectricasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.VoltajesInstalacionesElectricasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<VoltajesInstalacionesElectricasEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            voltajesInstalacionesElectricasEntidad = null;
