...
public partial class GruposRiesgosDeudores : System.Web.UI.Page
{
    #region PROPIEDADES
    ...
    private ConsultasWS.GruposRiesgosDeudoresEntidad _gruposRiesgosDeudoresEntidad = new ConsultasWS.GruposRiesgosDeudoresEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Grupo_Riesgo_Deudor";
                    consultaEntidad.ColumnaOrdenar = "Cod_Grupo_Riesgo_Deudor";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdGrupoRiesgoDeudor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGrupoRiesgoDeudor", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodGrupoRiesgoDeudor", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesGrupoRiesgoDeudor", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Grupo_Riesgo_Deudor"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Grupo_Riesgo_Deudor"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2)
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGrupoRiesgoDeudor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _gruposRiesgosDeudoresEntidad.IdGrupoRiesgoDeudor = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(_gruposRiesgosDeudoresEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(GruposRiesgosDeudoresEntidad entidad)
    {
        return wsConsultas.GruposRiesgosDeudoresEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.GruposRiesgosDeudoresTotalFilas(entidad);
    }

    private List<GruposRiesgosDeudoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.GruposRiesgosDeudoresConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<GruposRiesgosDeudoresEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool disponible)
    {
        if (!disponible)
        {
            if (disponible)
            ...
            _gruposRiesgosDeudoresEntidad = null;
