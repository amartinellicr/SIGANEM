...
public partial class TiposIngresos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposIngresosEntidad tiposIngresosEntidad = new ConsultasWS.TiposIngresosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Ingreso";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Ingreso";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoIngreso" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoIngreso", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoIngreso", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoIngreso", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Ingreso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Ingreso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoIngreso");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposIngresosEntidad.IdTipoIngreso = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposIngresosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposIngresosEntidad _entidad)
    {
        return wsConsultas.TiposIngresosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposIngresosTotalFilas(_entidad);
    }

    private List<TiposIngresosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposIngresosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposIngresosEntidad> _lista)
    {
