...
public partial class GruposFinancieros : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.GruposFinancierosEntidad  gruposFinancierosEntidad = new ConsultasWS.GruposFinancierosEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Grupo_Financiero";
					consultaEntidad.ColumnaOrdenar = "Cod_Grupo_Financiero";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdGrupoFinanciero" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		TemplateField lblID1 = new TemplateField();
		 gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGrupoFinanciero", lblID1);

		TemplateField lblID2 = new TemplateField();
		 gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoGrupoFinanciero", lblID2);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodGrupoFinanciero", string.Empty, "Código Grupo Financiero", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesGrupoFinanciero", string.Empty, "Descripción Grupo Financiero", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoGrupoFinanciero", string.Empty, "Código Tipo Grupo Financiero", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoGrupoFinanciero", string.Empty, "Descripción Tipo Grupo Financiero", HorizontalAlign.Center, false, true));
	}

	private void LimpiarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código Grupo Financiero", "Cod_Grupo_Financiero"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción Grupo Financiero", "Des_Grupo_Financiero"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGrupoFinanciero");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						 gruposFinancierosEntidad.IdGrupoFinanciero = int.Parse(lbl.Text);

						#endregion

                        _result = this.Eliminar( gruposFinancierosEntidad);
					}
				}
                ...
                this.mpeInformarBox.Show();
				int  valor = _result.ValorEstado;
				int  valorErr = _result.ValorError;
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(GruposFinancierosEntidad  entidad)
	{
		return wsConsultas.GruposFinancierosEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
	{
		return wsConsultas.GruposFinancierosTotalFilas( entidad);
	}

	private List<GruposFinancierosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
	{
		return wsConsultas.GruposFinancierosConsultar( entidad).ToList();
	}
    ...
    private void BindGridView(GridView  gridView, List<GruposFinancierosEntidad>  lista)
    {
