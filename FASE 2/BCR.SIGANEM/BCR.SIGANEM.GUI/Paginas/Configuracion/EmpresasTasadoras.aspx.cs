...
public partial class EmpresasTasadoras : System.Web.UI.Page
{
    ...
    private ConsultasWS.TasadoresEntidad _empresasTasadorasEntidad = new ConsultasWS.TasadoresEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Tasador";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Tasador";
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTasador", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoPersona", string.Empty, "Tipo Persona Empresa Tasadora", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTasador", string.Empty, "Id Empresa Tasadora", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesNombreTasador", string.Empty, "Nombre Empresa Tasadora", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Id Empresa", "Cod_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre Empresa", "Nombre_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Persona Empresa", "Des_Tipo_Persona"));
    }
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _empresasTasadorasEntidad.IdTasador = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_empresasTasadorasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TasadoresEntidad _entidad)
    {
        return wsConsultas.EmpresasTasadorasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.EmpresasTasadorasTotalFilas(_entidad);
    }

    private List<TasadoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.EmpresasTasadorasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TasadoresEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            _empresasTasadorasEntidad = null;
