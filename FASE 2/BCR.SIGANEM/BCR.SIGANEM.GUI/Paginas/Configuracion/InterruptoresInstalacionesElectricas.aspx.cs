...
public partial class InterruptoresInstalacionesElectricas : System.Web.UI.Page
{
    ...
    private ConsultasWS.InterruptoresInstalacionesElectricasEntidad interruptoresInstalacionesElectricasEntidad = new ConsultasWS.InterruptoresInstalacionesElectricasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Interruptor_Instalacion_Electrica";
                    consultaEntidad.ColumnaOrdenar = "Cod_Interruptor_Instalacion_Electrica";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdInterruptorInstalacionElectrica" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdInterruptorInstalacionElectrica", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodInterruptorInstalacionElectrica", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesInterruptorInstalacionElectrica", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Interruptor_Instalacion_Electrica"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Interruptor_Instalacion_Electrica"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdInterruptorInstalacionElectrica");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        interruptoresInstalacionesElectricasEntidad.IdInterruptorInstalacionElectrica = int.Parse(lbl.Text);

                        #endregion

                        result = this.Eliminar(interruptoresInstalacionesElectricasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(InterruptoresInstalacionesElectricasEntidad entidad)
    {
        return wsConsultas.InterruptoresInstalacionesElectricasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.InterruptoresInstalacionesElectricasTotalFilas(entidad);
    }

    private List<InterruptoresInstalacionesElectricasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.InterruptoresInstalacionesElectricasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<InterruptoresInstalacionesElectricasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            interruptoresInstalacionesElectricasEntidad = null;
