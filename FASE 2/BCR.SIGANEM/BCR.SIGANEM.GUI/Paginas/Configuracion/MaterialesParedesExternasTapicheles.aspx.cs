...
public partial class MaterialesParedesExternasTapicheles : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesExternosTapichelesEntidad materialesParedesExternasTapichelesEntidad = new ConsultasWS.MaterialesExternosTapichelesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Externo_Tapichel";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Externo_Tapichel";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMaterialExternoTapichel" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMaterialExternoTapichel", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodMaterialExternoTapichel", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesMaterialExternoTapichel", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Externo_Tapichel"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Externo_Tapichel"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMaterialExternoTapichel");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesParedesExternasTapichelesEntidad.IdMaterialExternoTapichel = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesParedesExternasTapichelesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesExternosTapichelesEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasTapichelesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasTapichelesTotalFilas(entidad);
    }

    private List<MaterialesExternosTapichelesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesParedesExternasTapichelesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MaterialesExternosTapichelesEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            materialesParedesExternasTapichelesEntidad = null;
