...
public partial class Colindantes : System.Web.UI.Page
{
    ...
    private ConsultasWS.ColindantesEntidad colindantesEntidad = new ConsultasWS.ColindantesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Colindante";
                    consultaEntidad.ColumnaOrdenar = "Cod_Colindante";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdColindante" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdColindante", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodColindante", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesColindante", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Colindante"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Colindante"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdColindante");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        colindantesEntidad.IdColindante = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(colindantesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ColindantesEntidad entidad)
    {
        return wsConsultas.ColindantesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.ColindantesTotalFilas(entidad);
    }

    private List<ColindantesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.ColindantesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<ColindantesEntidad> lista)
    {
