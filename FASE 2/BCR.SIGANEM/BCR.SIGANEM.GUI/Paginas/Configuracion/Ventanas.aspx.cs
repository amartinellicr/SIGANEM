...
public partial class Ventanas : System.Web.UI.Page
{
    ...
    private ConsultasWS.VentanasEntidad ventanasEntidad = new ConsultasWS.VentanasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Ventana";
                    consultaEntidad.ColumnaOrdenar = "Cod_Ventana";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdVentana" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdVentana", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodVentana", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesVentana", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Ventana"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Ventana"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdVentana");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        ventanasEntidad.IdVentana = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(ventanasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(VentanasEntidad _entidad)
    {
        return wsConsultas.VentanasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.VentanasTotalFilas(_entidad);
    }

    private List<VentanasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.VentanasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<VentanasEntidad> _lista)
    {
