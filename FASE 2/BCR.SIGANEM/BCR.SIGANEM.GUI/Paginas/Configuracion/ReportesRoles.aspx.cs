...
public partial class ReportesRoles : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.ReportesRolesEntidad reportesRolesEntidad = new ConsultasWS.ReportesRolesEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Reporte_Rol";
					consultaEntidad.ColumnaOrdenar = "Cod_Reporte_Rol";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdReporteRol" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdReporteRol", lblID);

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodReporteRol", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesReporteRol", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Reporte_Rol"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Reporte_Rol"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdReporteRol");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						reportesRolesEntidad.IdReporteRol = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(reportesRolesEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ReportesRolesEntidad entidad)
	{
		return wsConsultas.ReportesRolesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.ReportesRolesTotalFilas(entidad);
	}

	private List<ReportesRolesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.ReportesRolesConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<ReportesRolesEntidad> lista)
    {
