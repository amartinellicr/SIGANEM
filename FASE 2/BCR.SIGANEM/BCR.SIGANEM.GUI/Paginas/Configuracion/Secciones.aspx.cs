...
public partial class Secciones : System.Web.UI.Page
{
	...
	private ConsultasWS.SeccionesEntidad seccionesEntidad = new ConsultasWS.SeccionesEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cantidad_Seccion";
					consultaEntidad.ColumnaOrdenar = "Cantidad_Seccion";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdSeccion" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdSeccion", lblID);

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CantidadSeccion", string.Empty, "Cantidad", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Cantidad", "Cantidad_Seccion"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdSeccion");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						seccionesEntidad.IdSeccion = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(seccionesEntidad);
					}
				}
			...
			if (sesion.Codigo == 0)
			{
				...
				switch (e.CommandName)
				{
					...
					if (Int32.Parse(this.txtSlide.Text) <= this.Registros)
					{
						...
						consultaEntidad.ColumnaOrdenar = "Cod_Seccion";
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(SeccionesEntidad entidad)
	{
		return wsConsultas.SeccionesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.SeccionesTotalFilas(entidad);
	}

	private List<SeccionesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.SeccionesConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<SeccionesEntidad> lista)
    {
