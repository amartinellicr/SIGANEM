...
public partial class TenenciasPRT15 : System.Web.UI.Page
{
    ...
    private ConsultasWS.TenenciasPrt15Entidad tenenciasPrt15Entidad = new ConsultasWS.TenenciasPrt15Entidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tenencia_PRT_15";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tenencia_PRT_15";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTenenciaPRT15" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTenenciaPRT15", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTenenciaPRT15", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTenenciaPRT15", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tenencia_PRT_15"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tenencia_PRT_15"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTenenciaPRT15");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tenenciasPrt15Entidad.IdTenenciaPrt15 = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tenenciasPrt15Entidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TenenciasPrt15Entidad entidad)
    {
        return wsConsultas.TenenciasPRT15Eliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TenenciasPRT15TotalFilas(entidad);
    }

    private List<TenenciasPrt15Entidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TenenciasPRT15Consultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<TenenciasPrt15Entidad> lista)
    {
