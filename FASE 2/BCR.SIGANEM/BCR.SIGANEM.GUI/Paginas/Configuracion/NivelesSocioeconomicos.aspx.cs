...
public partial class NivelesSocioeconomicos : System.Web.UI.Page
{
    ...
    private ConsultasWS.NivelesSocioeconomicosEntidad nivelesSocioeconomicosEntidad = new ConsultasWS.NivelesSocioeconomicosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Nivel_Socioeconomico";
                    consultaEntidad.ColumnaOrdenar = "Cod_Nivel_Socioeconomico";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdNivelSocioeconomico" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdNivelSocioeconomico", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodNivelSocioeconomico", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesNivelSocioeconomico", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Nivel_Socioeconomico"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Nivel_Socioeconomico"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdNivelSocioeconomico");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        nivelesSocioeconomicosEntidad.IdNivelSocioeconomico = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(nivelesSocioeconomicosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(NivelesSocioeconomicosEntidad  entidad)
    {
        return wsConsultas.NivelesSocioeconomicosEliminar( entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad  entidad)
    {
        return wsConsultas.NivelesSocioeconomicosTotalFilas( entidad);
    }

    private List<NivelesSocioeconomicosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad  entidad)
    {
        return wsConsultas.NivelesSocioeconomicosConsultar( entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<NivelesSocioeconomicosEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            nivelesSocioeconomicosEntidad = null;
