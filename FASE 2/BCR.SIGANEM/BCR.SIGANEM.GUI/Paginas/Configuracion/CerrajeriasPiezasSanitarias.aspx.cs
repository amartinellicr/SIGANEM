...
public partial class CerrajeriasPiezasSanitarias : System.Web.UI.Page
{
    ...
    private ConsultasWS.CerrajeriasPiezasSanitariasEntidad cerrajeriasPiezasSanitariasEntidad = new ConsultasWS.CerrajeriasPiezasSanitariasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Cerrajeria_Pieza_Sanitaria";
                    consultaEntidad.ColumnaOrdenar = "Cod_Cerrajeria_Pieza_Sanitaria";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCerrajeriaPiezaSanitaria" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCerrajeriaPiezaSanitaria", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCerrajeriaPiezaSanitaria", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCerrajeriaPiezaSanitaria", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Cerrajeria_Pieza_Sanitaria"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Cerrajeria_Pieza_Sanitaria"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        ...
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCerrajeriaPiezaSanitaria");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cerrajeriasPiezasSanitariasEntidad.IdCerrajeriaPiezaSanitaria = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cerrajeriasPiezasSanitariasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CerrajeriasPiezasSanitariasEntidad entidad)
    {
        return wsConsultas.CerrajeriasPiezasSanitariasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CerrajeriasPiezasSanitariasTotalFilas(entidad);
    }

    private List<CerrajeriasPiezasSanitariasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CerrajeriasPiezasSanitariasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CerrajeriasPiezasSanitariasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            cerrajeriasPiezasSanitariasEntidad = null;
