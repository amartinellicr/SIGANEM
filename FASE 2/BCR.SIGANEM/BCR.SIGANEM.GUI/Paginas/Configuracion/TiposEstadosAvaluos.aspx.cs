...
public partial class TiposEstadosAvaluos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposEstadosAvaluosEntidad tiposEstadosAvaluosEntidad = new ConsultasWS.TiposEstadosAvaluosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Estado_Avaluo";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Estado_Avaluo";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoEstadoAvaluo" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoEstadoAvaluo", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoEstadoAvaluo", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoEstadoAvaluo", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Estado_Avaluo"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Estado_Avaluo"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoEstadoAvaluo");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposEstadosAvaluosEntidad.IdTipoEstadoAvaluo = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposEstadosAvaluosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposEstadosAvaluosEntidad _entidad)
    {
        return wsConsultas.TiposEstadosAvaluosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposEstadosAvaluosTotalFilas(_entidad);
    }

    private List<TiposEstadosAvaluosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposEstadosAvaluosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposEstadosAvaluosEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            tiposEstadosAvaluosEntidad = null;
