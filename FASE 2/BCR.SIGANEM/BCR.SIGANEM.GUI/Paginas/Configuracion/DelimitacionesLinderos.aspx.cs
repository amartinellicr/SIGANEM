...
public partial class DelimitacionesLinderos : System.Web.UI.Page
{
    ...
    private ConsultasWS.DelimitacionesLinderosEntidad delimitacionesLinderosEntidad = new ConsultasWS.DelimitacionesLinderosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Delimitacion_Lindero";
                    consultaEntidad.ColumnaOrdenar = "Cod_Delimitacion_Lindero";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdDelimitacionLindero" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdDelimitacionLindero", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodDelimitacionLindero", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesDelimitacionLindero", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Delimitacion_Lindero"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Delimitacion_Lindero"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdDelimitacionLindero");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        delimitacionesLinderosEntidad.IdDelimitacionLindero = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(delimitacionesLinderosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(DelimitacionesLinderosEntidad entidad)
    {
        return wsConsultas.DelimitacionesLinderosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.DelimitacionesLinderosTotalFilas(entidad);
    }

    private List<DelimitacionesLinderosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.DelimitacionesLinderosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<DelimitacionesLinderosEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            delimitacionesLinderosEntidad = null;
