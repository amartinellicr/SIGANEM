...
public partial class ViasAccesos : System.Web.UI.Page
{
    ...
    private ConsultasWS.ViasAccesoEntidad viasAccesoEntidad = new ConsultasWS.ViasAccesoEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Via_Acceso";
                    consultaEntidad.ColumnaOrdenar = "Cod_Via_Acceso";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdViaAcceso" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdViaAcceso", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodViaAcceso", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesViaAcceso", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Via_Acceso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Via_Acceso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdViaAcceso");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        viasAccesoEntidad.IdViaAcceso = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(viasAccesoEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ViasAccesoEntidad _entidad)
    {
        return wsConsultas.ViasAccesosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.ViasAccesosTotalFilas(_entidad);
    }

    private List<ViasAccesoEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.ViasAccesosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<ViasAccesoEntidad> _lista)
    {
