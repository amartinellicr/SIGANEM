...
public partial class TenenciasPRT17 : System.Web.UI.Page
{
    ...
    private ConsultasWS.TenenciasPrt17Entidad tenenciasPrt17Entidad = new ConsultasWS.TenenciasPrt17Entidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tenencia_PRT_17";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tenencia_PRT_17";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTenenciaPRT17" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTenenciaPRT17", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTenenciaPRT17", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTenenciaPRT17", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tenencia_PRT_17"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tenencia_PRT_17"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTenenciaPRT17");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tenenciasPrt17Entidad.IdTenenciaPrt17 = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tenenciasPrt17Entidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TenenciasPrt17Entidad entidad)
    {
        return wsConsultas.TenenciasPRT17Eliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.TenenciasPRT17TotalFilas(entidad);
    }

    private List<TenenciasPrt17Entidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.TenenciasPRT17Consultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<TenenciasPrt17Entidad> lista)
    {
