...
public partial class Instrumentos : System.Web.UI.Page
{
    ...
    private ConsultasWS.InstrumentosEntidad instrumentosEntidad = new ConsultasWS.InstrumentosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Instrumento";
                    consultaEntidad.ColumnaOrdenar = "Cod_Instrumento";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdInstrumento" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdInstrumento", lblID1);

        TemplateField lblID2 = new TemplateField();
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoInstrumento", lblID2);
        ...
        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoInstrumento", string.Empty, "Código Tipo Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoInstrumento", string.Empty, "Descripción Tipo Instrumento", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código Instrumento", "Cod_Instrumento"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción Instrumento", "Des_Instrumento"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdInstrumento");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        instrumentosEntidad.IdInstrumento = int.Parse(lbl.Text);

                        #endregion

                        result = this.Eliminar(instrumentosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(InstrumentosEntidad entidad)
    {
        return wsConsultas.InstrumentosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.InstrumentosTotalFilas(entidad);
    }

    private List<InstrumentosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.InstrumentosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<InstrumentosEntidad> lista)
    {
