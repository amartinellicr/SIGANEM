...
public partial class EmpresasCalificadoras : System.Web.UI.Page
{
    ...
    private ConsultasWS.EmpresasCalificadorasEntidad empresasCalEntidad = new ConsultasWS.EmpresasCalificadorasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Empresa_Calificadora";
                    consultaEntidad.ColumnaOrdenar = "Cod_Empresa_Calificadora";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEmpresaCalificadora" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEmpresaCalificadora", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEmpresaCalificadora", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEmpresaCalificadora", string.Empty, "Nombre", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesPlazoCalificacion", string.Empty, "Plazo Calificación", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código Empresa", "Cod_Empresa_Calificadora"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre Empresa", "Des_Empresa_Calificadora"));
        this.ddlFiltro.Items.Add(new ListItem("Plazo Calificación", "Des_Plazo_Calificacion"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEmpresaCalificadora");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        empresasCalEntidad.IdEmpresaCalificadora = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(empresasCalEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EmpresasCalificadorasEntidad entidad)
    {
        return wsConsultas.EmpresasCalificadorasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.EmpresasCalificadorasTotalFilas(entidad);
    }

    private List<EmpresasCalificadorasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.EmpresasCalificadorasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<EmpresasCalificadorasEntidad> lista)
    {
