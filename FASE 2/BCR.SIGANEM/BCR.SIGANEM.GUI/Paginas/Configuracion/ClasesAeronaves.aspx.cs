...
public partial class ClasesAeronaves : System.Web.UI.Page
{
    ...
    private ConsultasWS.ClasesAeronavesEntidad clasesAeronavesEntidad = new ConsultasWS.ClasesAeronavesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Clase_Aeronave";
                    consultaEntidad.ColumnaOrdenar = "Cod_Clase_Aeronave";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdClaseAeronave" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdClaseAeronave", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodClaseAeronave", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesClaseAeronave", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Clase_Aeronave"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Clase_Aeronave"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdClaseAeronave");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        clasesAeronavesEntidad.IdClaseAeronave = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(clasesAeronavesEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ClasesAeronavesEntidad entidad)
    {
        return wsConsultas.ClasesAeronavesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.ClasesAeronavesTotalFilas(entidad);
    }

    private List<ClasesAeronavesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.ClasesAeronavesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<ClasesAeronavesEntidad> lista)
    {
