...
public partial class MaterialesPuertas : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesPuertasEntidad materialesPuertasEntidad = new ConsultasWS.MaterialesPuertasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Puerta";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Puerta";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMaterialPuerta" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMaterialPuerta", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodMaterialPuerta", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesMaterialPuerta", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Puerta"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Puerta"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMaterialPuerta");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesPuertasEntidad.IdMaterialPuerta = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesPuertasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesPuertasEntidad entidad)
    {
        return wsConsultas.MaterialesPuertasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesPuertasTotalFilas(entidad);
    }

    private List<MaterialesPuertasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesPuertasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MaterialesPuertasEntidad> lista)
    {
