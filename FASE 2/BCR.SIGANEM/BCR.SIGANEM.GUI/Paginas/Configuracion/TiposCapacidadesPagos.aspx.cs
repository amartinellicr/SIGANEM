...
public partial class TiposCapacidadesPagos : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposCapacidadesPagosEntidad tiposCapacidadesPagosEntidad = new ConsultasWS.TiposCapacidadesPagosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Capacidad_Pago";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Capacidad_Pago";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoCapacidadPago" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoCapacidadPago", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoCapacidadPago", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoCapacidadPago", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Capacidad_Pago"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Capacidad_Pago"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoCapacidadPago");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposCapacidadesPagosEntidad.IdTipoCapacidadPago = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposCapacidadesPagosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposCapacidadesPagosEntidad _entidad)
    {
        return wsConsultas.TiposCapacidadesPagosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposCapacidadesPagosTotalFilas(_entidad);
    }

    private List<TiposCapacidadesPagosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposCapacidadesPagosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposCapacidadesPagosEntidad> _list)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            ...
            tiposCapacidadesPagosEntidad = null;
