...
public partial class EstadosAvaluos : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.EstadosAvaluosEntidad _estadosAvaluosEntidad = new ConsultasWS.EstadosAvaluosEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())//CAMBIAR SESION-AGREGAR
			{
				...
				if (!Page.IsPostBack)
				{
					...
					_consultaEntidad.ColumnaFiltro = "Cod_Estado_Avaluo";
					_consultaEntidad.ColumnaOrdenar = "Cod_Estado_Avaluo";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEstadoAvaluo" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEstadoAvaluo", lblID1);

		TemplateField lblID2 = new TemplateField();
		_gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoEstadoAvaluo", lblID2);

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEstadoAvaluo", string.Empty, "Código Estado Avalúo", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEstadoAvaluo", string.Empty, "Descripción Estado Avalúo", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoEstadoAvaluo", string.Empty, "Código Tipo de Estado", HorizontalAlign.Center, false, true));

		this.gridViewColumna = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoEstadoAvaluo", string.Empty, "Descripción Tipo de Estado", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Estado_Avaluo"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Estado_Avaluo"));
	}
				...
				if (Page.IsPostBack)
				{
					if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEstadoAvaluo");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						_estadosAvaluosEntidad.IdEstadoAvaluo = int.Parse(lbl.Text);

						#endregion

                        _result = this.Eliminar(_estadosAvaluosEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EstadosAvaluosEntidad _entidad)
	{
		return wsConsultas.EstadosAvaluosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
	{
		return wsConsultas.EstadosAvaluosTotalFilas(_entidad);
	}

	private List<EstadosAvaluosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
	{
		return wsConsultas.EstadosAvaluosConsultar(_entidad).ToList();
	}
    ...
    private void BindGridView(GridView _gridView, List<EstadosAvaluosEntidad> _lista)
    {
