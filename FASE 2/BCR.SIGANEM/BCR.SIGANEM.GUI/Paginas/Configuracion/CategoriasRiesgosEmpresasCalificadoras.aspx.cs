...
public partial class CategoriasRiesgosEmpresasCalificadoras : System.Web.UI.Page
{
    ...
    private ConsultasWS.CategoriasRiesgosEmpresasCalificadorasEntidad categoriasRiesgosEmpresasCalificadorasEntidad = new ConsultasWS.CategoriasRiesgosEmpresasCalificadorasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Categoria_Riesgo_Empresa_Calificadora";
                    consultaEntidad.ColumnaOrdenar = "Cod_Categoria_Riesgo_Empresa_Calificadora";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCategoriaRiesgoEmpresaCalificadora" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCategoriaRiesgoEmpresaCalificadora", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCategoriaRiesgoEmpresaCalificadora", string.Empty, "Código", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Categoria_Riesgo_Empresa_Calificadora"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCategoriaRiesgoEmpresaCalificadora");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        categoriasRiesgosEmpresasCalificadorasEntidad.IdCategoriaRiesgoEmpresaCalificadora = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(categoriasRiesgosEmpresasCalificadorasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CategoriasRiesgosEmpresasCalificadorasEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosEmpresasCalificadorasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosEmpresasCalificadorasTotalFilas(entidad);
    }

    private List<CategoriasRiesgosEmpresasCalificadorasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosEmpresasCalificadorasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CategoriasRiesgosEmpresasCalificadorasEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            categoriasRiesgosEmpresasCalificadorasEntidad = null;
