...
public partial class MaterialesPisos : System.Web.UI.Page
{
    ...
    private ConsultasWS.MaterialesPisosEntidad materialesPisosEntidad = new ConsultasWS.MaterialesPisosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Material_Piso";
                    consultaEntidad.ColumnaOrdenar = "Cod_Material_Piso";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdMaterialPiso" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMaterialPiso", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodMaterialPiso", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesMaterialPiso", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Material_Piso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Material_Piso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMaterialPiso");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        materialesPisosEntidad.IdMaterialPiso = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(materialesPisosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(MaterialesPisosEntidad entidad)
    {
        return wsConsultas.MaterialesPisosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.MaterialesPisosTotalFilas(entidad);
    }

    private List<MaterialesPisosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.MaterialesPisosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<MaterialesPisosEntidad> lista)
    {
