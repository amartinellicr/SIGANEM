...
public partial class Topografias : System.Web.UI.Page
{
    ...
    private ConsultasWS.TopografiasEntidad topografiasEntidad = new ConsultasWS.TopografiasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Topografia";
                    consultaEntidad.ColumnaOrdenar = "Cod_Topografia";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTopografia" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTopografia", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTopografia", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTopografia", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Topografia"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Topografia"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        ...
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTopografia");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        topografiasEntidad.IdTopografia = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(topografiasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TopografiasEntidad _entidad)
    {
        return wsConsultas.TopografiasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TopografiasTotalFilas(_entidad);
    }

    private List<TopografiasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TopografiasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TopografiasEntidad> _lista)
    {
