...
public partial class Derechos : System.Web.UI.Page
{
    ...
    private ConsultasWS.DerechosEntidad derechosEntidad = new ConsultasWS.DerechosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Derecho";
                    consultaEntidad.ColumnaOrdenar = "Cod_Derecho";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdDerecho" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdDerecho", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodDerecho", string.Empty, "Código", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Derecho"));
        //this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Derecho"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdDerecho");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        derechosEntidad.IdDerecho = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(derechosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(DerechosEntidad entidad)
    {
        return wsConsultas.DerechosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.DerechosTotalFilas(entidad);
    }

    private List<DerechosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.DerechosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<DerechosEntidad> lista)
    {
