...
public partial class CantidadesFincas : System.Web.UI.Page
{
    ...
    private ConsultasWS.CantidadesFincasEntidad cantidadesFincasEntidad = new ConsultasWS.CantidadesFincasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cantidad_Finca";
                    consultaEntidad.ColumnaOrdenar = "Cantidad_Finca";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCantidadFinca" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCantidadFinca", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CantidadFinca", string.Empty, "Cantidad de Fincas", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Cantidad de Finca", "Cantidad_Finca"));
        //this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Activo"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCantidadFinca");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cantidadesFincasEntidad.IdCantidadFinca = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cantidadesFincasEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CantidadesFincasEntidad entidad)
    {
        return wsConsultas.CantidadesFincasEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CantidadesFincasTotalFilas(entidad);
    }

    private List<CantidadesFincasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CantidadesFincasConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CantidadesFincasEntidad> lista)
    {
