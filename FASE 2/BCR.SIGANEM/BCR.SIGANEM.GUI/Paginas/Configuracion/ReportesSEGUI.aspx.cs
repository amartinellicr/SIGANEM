...
public partial class ReportesSEGUI : System.Web.UI.Page
{
	#region PROPIEDADES
	...
	private ConsultasWS.ReportesSeguiEntidad reportesSEGUIEntidad = new ConsultasWS.ReportesSeguiEntidad();
		...
		try
		{
			...
			if (AccesoPermisoPagina())
			{
				...
				if (!Page.IsPostBack)
				{
					...
					consultaEntidad.ColumnaFiltro = "Cod_Reporte_SEGUI";
					consultaEntidad.ColumnaOrdenar = "Cod_Reporte_SEGUI";
				...
				// ASIGNA DATA KEYS
                String[] DataKeysString = { "IdReporteSegui" };
	...
	private void gridView_Init(object sender, EventArgs e)
	{
		...
		gvTemplate.CrearCamposGridNoVisibles(gridView, "IdReporteSegui", lblID);

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodReporteSegui", string.Empty, "Código", HorizontalAlign.Center, false, true));

		this.gridViewColumn = new GridViewColumn();
		this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesReporteSegui", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
	}

	private void CargarDDLFiltro()
	{
		// CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Reporte_SEGUI"));
		this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Reporte_SEGUI"));
	}
				...
				if (Page.IsPostBack)
				{
					...
					if (ContadorSeleccionados() < 2) 
					{
						foreach (GridViewRow row in gridView.Rows)
						{
							...
							{
								Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdReporteSegui");
			...
			if (sesion.Codigo == 0)
			{
				...
				foreach (GridViewRow row in gridView.Rows)
				{
					...
					if (checkBoxColumn.Checked)
					{
						...
						pantallaIdOculto.Value = lbl.Text;
						reportesSEGUIEntidad.IdReporteSegui = int.Parse(lbl.Text);

						#endregion

                        resultado = this.Eliminar(reportesSEGUIEntidad);
					}
				}
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ReportesSeguiEntidad entidad)
	{
		return wsConsultas.ReportesSEGUIEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
	}

	private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
	{
		return wsConsultas.ReportesSEGUITotalFilas(entidad);
	}

	private List<ReportesSeguiEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
	{
		return wsConsultas.ReportesSEGUIConsultar(entidad).ToList();
	}
    ...
    private void BindGridView(GridView gridView, List<ReportesSeguiEntidad> lista)
    {
