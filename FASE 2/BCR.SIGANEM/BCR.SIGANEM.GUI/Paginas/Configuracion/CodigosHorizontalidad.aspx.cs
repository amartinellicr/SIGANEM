...
public partial class CodigosHorizontalidad : System.Web.UI.Page
{
    ...
    private ConsultasWS.CodigosHorizontalidadEntidad codigosHorizontalidadEntidad = new ConsultasWS.CodigosHorizontalidadEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Codigo_Horizontalidad";
                    consultaEntidad.ColumnaOrdenar = "Cod_Codigo_Horizontalidad";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCodigoHorizontalidad" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCodigoHorizontalidad", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCodigoHorizontalidad", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCodigoHorizontalidad", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Codigo_Horizontalidad"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Codigo_Horizontalidad"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCodigoHorizontalidad");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        codigosHorizontalidadEntidad.IdCodigoHorizontalidad = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(codigosHorizontalidadEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CodigosHorizontalidadEntidad entidad)
    {
        return wsConsultas.CodigosHorizontalidadEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CodigosHorizontalidadTotalFilas(entidad);
    }

    private List<CodigosHorizontalidadEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CodigosHorizontalidadConsultar(entidad).ToList();
    }
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            decimal resultRows = (filasContador / maxRows), rounded = Math.Ceiling(resultRows);
    ...
    private void BindGridView(GridView _gridView, List<CodigosHorizontalidadEntidad> lista)
    {
        _gridView.DataSource = lista;
        _gridView.DataBind();
    }
    ...
    private int ContadorSeleccionados()
    {
        ...
        foreach (GridViewRow row1 in gridView.Rows)
        {
            ...
            if (checkBoxColumn.Checked == true)
            {
                cantidad++;
            }
        }
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            codigosHorizontalidadEntidad = null;
