...
public partial class CubiertasTechos : System.Web.UI.Page
{
    ...
    private ConsultasWS.CubiertasTechosEntidad cubiertasTechosEntidad = new ConsultasWS.CubiertasTechosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Cubierta_Techo";
                    consultaEntidad.ColumnaOrdenar = "Cod_Cubierta_Techo";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCubiertaTecho" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCubiertaTecho", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCubiertaTecho", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCubiertaTecho", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Cubierta_Techo"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Cubierta_Techo"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCubiertaTecho");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cubiertasTechosEntidad.IdCubiertaTecho = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cubiertasTechosEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CubiertasTechosEntidad entidad)
    {
        return wsConsultas.CubiertasTechosEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CubiertasTechosTotalFilas(entidad);
    }

    private List<CubiertasTechosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CubiertasTechosConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CubiertasTechosEntidad> lista)
    {
