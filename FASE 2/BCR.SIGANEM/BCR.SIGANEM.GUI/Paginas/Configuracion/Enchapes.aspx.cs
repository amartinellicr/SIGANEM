...
public partial class Enchapes : System.Web.UI.Page
{
    ...
    private ConsultasWS.EnchapesEntidad enchapesEntidad = new ConsultasWS.EnchapesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Enchape";
                    consultaEntidad.ColumnaOrdenar = "Cod_Enchape";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdEnchape" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdEnchape", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodEnchape", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEnchape", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Enchape"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Enchape"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdEnchape");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        enchapesEntidad.IdEnchape = int.Parse(lbl.Text);

                        #endregion

                         resultado = this.Eliminar(enchapesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(EnchapesEntidad entidad)
    {
        return wsConsultas.EnchapesEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.EnchapesTotalFilas(entidad);
    }

    private List<EnchapesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.EnchapesConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<EnchapesEntidad> lista)
    {
