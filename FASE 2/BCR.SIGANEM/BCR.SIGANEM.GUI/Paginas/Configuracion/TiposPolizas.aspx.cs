...
public partial class TiposPolizas : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposPolizasEntidad tiposPolizasEntidad = new ConsultasWS.TiposPolizasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Poliza";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Poliza";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoPoliza" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoPoliza", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoPoliza", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoPoliza", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Poliza"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Poliza"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoPoliza");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposPolizasEntidad.IdTipoPoliza = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposPolizasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposPolizasEntidad _entidad)
    {
        return wsConsultas.TiposPolizasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposPolizasTotalFilas(_entidad);
    }

    private List<TiposPolizasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposPolizasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposPolizasEntidad> _lista)
    {
