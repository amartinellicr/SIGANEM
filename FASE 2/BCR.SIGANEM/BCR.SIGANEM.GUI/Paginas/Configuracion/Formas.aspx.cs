...
public partial class Formas : System.Web.UI.Page
{
    ...
    private ConsultasWS.FormasEntidad _formasEntidad = new ConsultasWS.FormasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    _consultaEntidad.ColumnaFiltro = "Cod_Forma";
                    _consultaEntidad.ColumnaOrdenar = "Cod_Forma";
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdForma", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodForma", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesForma", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Forma"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Forma"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdForma");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        _formasEntidad.IdForma = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(_formasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(FormasEntidad _entidad)
    {
        return wsConsultas.FormasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.FormasTotalFilas(_entidad);
    }

    private List<FormasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.FormasConsultar(_entidad).ToList();
    }
    ...
    protected ConsultasWS.BitacorasEntidad AsignarValoresBitacora(EnumTipoBitacora _tipo)
    {
        try
        {
            ...
            _bitacorasEntidad.CodAccion =  bitacoraFlags.TipoBitacoraConsulta(_tipo);
    ...
    private void BindGridView(GridView _gridView, List<FormasEntidad> _lista)
    {
