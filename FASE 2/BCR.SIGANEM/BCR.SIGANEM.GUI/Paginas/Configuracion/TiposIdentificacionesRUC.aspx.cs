...
public partial class TiposIdentificacionesRUC : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposIdentificacionesRUCEntidad tiposIdentificacionesRUCEntidad = new ConsultasWS.TiposIdentificacionesRUCEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Identificacion_RUC";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Identificacion_RUC";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoIdentificacionRUC" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoIdentificacionRUC", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoIdentificacionRUC", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoIdentificacionRUC", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Identificacion_RUC"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Identificacion_RUC"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoIdentificacionRUC");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposIdentificacionesRUCEntidad.IdTipoIdentificacionRUC = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tiposIdentificacionesRUCEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposIdentificacionesRUCEntidad _entidad)
    {
        return wsConsultas.TiposIdentificacionesRUCEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposIdentificacionesRUCTotalFilas(_entidad);
    }

    private List<TiposIdentificacionesRUCEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposIdentificacionesRUCConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposIdentificacionesRUCEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            tiposIdentificacionesRUCEntidad = null;
