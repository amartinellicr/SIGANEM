...
public partial class Provincias : System.Web.UI.Page
{
    ...
    private ConsultasWS.ProvinciasEntidad provinciasEntidad = new ConsultasWS.ProvinciasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Provincia";
                    consultaEntidad.ColumnaOrdenar = "Cod_Provincia";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdProvincia" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdProvincia", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodProvincia", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesProvincia", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Provincia"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Provincia"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdProvincia");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        provinciasEntidad.IdProvincia = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(provinciasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(ProvinciasEntidad _entidad)
    {
        return wsConsultas.ProvinciasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.ProvinciasTotalFilas(_entidad);
    }

    private List<ProvinciasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.ProvinciasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<ProvinciasEntidad> _lista)
    {
