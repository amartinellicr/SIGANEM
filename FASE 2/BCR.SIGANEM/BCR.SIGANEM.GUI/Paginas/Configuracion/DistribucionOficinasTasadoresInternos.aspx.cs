...
public partial class DistribucionOficinasTasadoresInternos : System.Web.UI.Page
{
    ...
    private ConsultasWS.DistribucionesZonasTasadoresEntidad distribucionZonasTasadoresInternosEntidad = new ConsultasWS.DistribucionesZonasTasadoresEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Distribucion_Zona_Tasador";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Distribucion_Zona_Tasador";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdDistribucionZonaTasador" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdDistribucionZonaTasador", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodZonaTasador", string.Empty, "Oficina", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesProvincia", string.Empty, "Provincia", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCanton", string.Empty, "Cantón", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesDistrito", string.Empty, "Distrito", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Oficina", "Cod_Zona_Tasador"));
        this.ddlFiltro.Items.Add(new ListItem("Provincia", "Des_Provincia"));
        this.ddlFiltro.Items.Add(new ListItem("Cantón", "Des_Canton"));
        this.ddlFiltro.Items.Add(new ListItem("Distrito", "Des_Distrito"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdDistribucionZonaTasador");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        distribucionZonasTasadoresInternosEntidad.IdDistribucionZonaTasador = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(distribucionZonasTasadoresInternosEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(DistribucionesZonasTasadoresEntidad entidad)
    {
        entidad.CodTipoDistribucionZonaTasador = "I";
        return wsConsultas.DistribucionZonasTasadoresEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.DistribucionZonasTasadoresTotalFilas(entidad, "I");
    }

    private List<DistribucionesZonasTasadoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.DistribucionZonasTasadoresConsultar(entidad, "I").ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<DistribucionesZonasTasadoresEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            distribucionZonasTasadoresInternosEntidad = null;
