...
public partial class TiposCarteras : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposCarterasEntidad tiposCarterasEntidad = new ConsultasWS.TiposCarterasEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Cartera";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Cartera";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoCartera" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoCartera", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoCartera", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoCartera", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Cartera"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Cartera"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoCartera");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposCarterasEntidad.IdTipoCartera = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposCarterasEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposCarterasEntidad _entidad)
    {
        return wsConsultas.TiposCarterasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposCarterasTotalFilas(_entidad);
    }

    private List<TiposCarterasEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposCarterasConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposCarterasEntidad> _lista)
    {
