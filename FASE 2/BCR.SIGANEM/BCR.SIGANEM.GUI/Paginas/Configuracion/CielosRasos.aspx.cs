...
public partial class CielosRasos : System.Web.UI.Page
{
    ...
    private ConsultasWS.CielosRasosEntidad cielosRasosEntidad = new ConsultasWS.CielosRasosEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Cielo_Raso";
                    consultaEntidad.ColumnaOrdenar = "Cod_Cielo_Raso";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCieloRaso" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCieloRaso", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCieloRaso", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCieloRaso", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Cielo_Raso"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Cielo_Raso"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCieloRaso");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        cielosRasosEntidad.IdCieloRaso = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(cielosRasosEntidad);
                    }
                }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CielosRasosEntidad _entidad)
    {
        return wsConsultas.CielosRasosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.CielosRasosTotalFilas(_entidad);
    }

    private List<CielosRasosEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.CielosRasosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CielosRasosEntidad> lista)
    {
