...
public partial class TiposBienes : System.Web.UI.Page
{
    ...
    private ConsultasWS.TiposBienesEntidad tiposBienesEntidad = new ConsultasWS.TiposBienesEntidad(); 
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Bien";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Bien";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdTipoBien" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoBien", lblID);

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("CodTipoBien", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumn = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumn.CreateBoundField("DesTipoBien", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Bien"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Bien"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoBien");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tiposBienesEntidad.IdTipoBien = int.Parse(lbl.Text);

                        #endregion

                        _result = this.Eliminar(tiposBienesEntidad);
                    }
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(TiposBienesEntidad _entidad)
    {
        return wsConsultas.TiposBienesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsConsultas.TiposBienesTotalFilas(_entidad);
    }

    private List<TiposBienesEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad _entidad)
    {
        return wsConsultas.TiposBienesConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<TiposBienesEntidad> _lista)
    {
