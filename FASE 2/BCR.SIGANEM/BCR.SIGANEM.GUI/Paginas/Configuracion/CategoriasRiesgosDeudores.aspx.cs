...
public partial class CategoriasRiesgosDeudores : System.Web.UI.Page
{
    ...
    private ConsultasWS.CategoriasRiesgosDeudoresEntidad categoriasRiesgosDeudoresEntidad = new ConsultasWS.CategoriasRiesgosDeudoresEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Categoria_Riesgo_Deudor";
                    consultaEntidad.ColumnaOrdenar = "Cod_Categoria_Riesgo_Deudor";
                ...
                // ASIGNA DATA KEYS
                String[] DataKeysString = { "IdCategoriaRiesgoDeudor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdCategoriaRiesgoDeudor", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodCategoriaRiesgoDeudor", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesCategoriaRiesgoDeudor", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Categoria_Riesgo_Deudor"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Categoria_Riesgo_Deudor"));
    }
                ...
                if (Page.IsPostBack)
                {
                    ...
                    if (ContadorSeleccionados() < 2) 
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdCategoriaRiesgoDeudor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        categoriasRiesgosDeudoresEntidad.IdCategoriaRiesgoDeudor = int.Parse(lbl.Text);

                        #endregion

                        result = this.Eliminar(categoriasRiesgosDeudoresEntidad);
                    }
                }
                ...
                int valorError = result.ValorError;
    ...
    private ConsultasWS.RespuestaEntidad Eliminar(CategoriasRiesgosDeudoresEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosDeudoresEliminar(entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(ConsultasWS.ParametrosTotalFilasEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosDeudoresTotalFilas(entidad);
    }

    private List<CategoriasRiesgosDeudoresEntidad> Consulta(ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.CategoriasRiesgosDeudoresConsultar(entidad).ToList();
    }
    ...
    private void BindGridView(GridView gridView, List<CategoriasRiesgosDeudoresEntidad> lista)
    {
    ...
    protected virtual void Dispose(bool eliminacion)
    {
        if (!disponible)
        {
            ...
            categoriasRiesgosDeudoresEntidad = null;
