...
public partial class BitacoraProcesos : System.Web.UI.Page
{
    ...
    private Button btnDescargaArchivo = null;
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        try
        {
            ...
            this.btnDescargaArchivo = ((Button)this.Master.FindControl("Ribbon1").FindControl("TabContainer1").FindControl("tabAcciones").FindControl("btnDescargaArchivo"));
            this.btnDescargaArchivo.Click += new EventHandler(btnDescargaArchivo_Click);
            this.btnDescargaArchivo.CausesValidation = false;
            ...
            // ASIGNA DATA KEYS
            String[] DataKeysString = { "IdBitacoraProceso" };
    ...
    protected void btnDescargaArchivo_Click(object sender, EventArgs e)
    {
        try
        {
            RespuestaConsultaSesion sesion = wsSesiones.ConsultarSesion(idSesionOculto.Value);
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    httpPost.RedirectAndPOST(this.Page, "DescargaArchivoSalidaSUGEF.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdBitacoraProceso", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesProceso", string.Empty, "Proceso", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEstado", string.Empty, "Resultado", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesFechaEjecucion", string.Empty, "Fecha y Hora de Ejecución", HorizontalAlign.Center, false, true));
    }
    ...
    protected void SetRegistrosLabelIndex()
    {
        try
        {
            ...
            decimal filasContador = this.TotalFilas(consulta);
    ...
    private bool ExisteError()
    {
        try
        {
            ...
            if (this.pantallaCodOculto.Value.Length > 0)
            {
                ...
                retornoValidacion = ValidarFechas();
            }
    ...
    private int ValidarFechas()
    {
        ...
        txtFechaInicio = (TextBox)this.tableData.FindControl("FechaDesde");
        txtFechaFinal = (TextBox)this.tableData.FindControl("FechaHasta");

        Label lblFechaInicio = null;
        lblFechaInicio = (Label)this.tableData.FindControl("lblFechaDesde");
        Label lblFechaFinal = null;
        lblFechaFinal = (Label)this.tableData.FindControl("lblFechaHasta");
    ...
    private Int32 TotalFilas(BitacoraProcesosConsulta entidad)
    {
        return wsConsultas.BitacoraProcesosTotalFilas(entidad);
    }

    private List<BitacoraProcesosDetalle> Consulta(BitacoraProcesosConsulta consulta, ConsultasWS.ParametrosConsultaEntidad entidad)
    {
        return wsConsultas.BitacoraProcesosConsultar(consulta, entidad).ToList();
    }
    ...
    protected Dictionary<string, string> SetRutaVentana()
    {
        ...
        SeguridadWS.PantallasEntidad pantallaDescarga = (from p in pantallas
                                                         where p.RutaPantalla.Contains("DescargaArchivoSalidaSUGEF.aspx")
                                                         select p).FirstOrDefault();

        Dictionary<string, string> data = new Dictionary<string, string>();
        data.Add("idSesion", idSesionOculto.Value);
        data.Add("codUsuario", codUsuarioOculto.Value);
        data.Add("pantallaModulo", pantallaDescarga.CodPantalla.ToString());
    ...
    private void BindGridView(GridView gridView, List<BitacoraProcesosDetalle> lista)
    {
