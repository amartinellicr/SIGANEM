...
public partial class Usuarios : System.Web.UI.Page
{
    ...
    private SeguridadWS.UsuariosEntidad usuariosEntidad = new SeguridadWS.UsuariosEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Usuario";
                    consultaEntidad.ColumnaOrdenar = "Cod_Usuario";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdUsuario" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdUsuario", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodUsuario", string.Empty, "Código Usuario", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("NombreUsuario", string.Empty, "Nombre de Usuario", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoRol", string.Empty, "Rol Asignado", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesConectado", string.Empty, "Conectado", HorizontalAlign.Center, false, false));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesBloqueado", string.Empty, "Bloqueado", HorizontalAlign.Center, false, true));
        
        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("UltimaConexion", string.Empty, "Ultima Conexión", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código Usuario", "Cod_Usuario"));
        this.ddlFiltro.Items.Add(new ListItem("Nombre Usuario", "Nombre_Usuario"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdUsuario");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        usuariosEntidad.IdUsuario = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(usuariosEntidad);
                    }
    ...
    private SeguridadWS.RespuestaEntidad Eliminar(UsuariosEntidad _entidad)
    {
        return wsSeguridad.UsuariosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(SeguridadWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsSeguridad.UsuariosTotalFilas(_entidad);
    }

    private List<UsuariosEntidad> Consulta(SeguridadWS.ParametrosConsultaEntidad _entidad)
    {
        return wsSeguridad.UsuariosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<UsuariosEntidad> _lista)
    {
