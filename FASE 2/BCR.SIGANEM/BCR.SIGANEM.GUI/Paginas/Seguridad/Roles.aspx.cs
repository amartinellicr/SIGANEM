...
public partial class Roles : System.Web.UI.Page
{
    ...
    private Button btnAcciones = null;
    private Button btnCopiarRol = null;

    //Req 1-24653624
    private Button btnEjecutarIPC = null;
    ...
    private SeguridadWS.TiposRolesEntidad rolesEntidad = new SeguridadWS.TiposRolesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Tipo_Rol";
                    consultaEntidad.ColumnaOrdenar = "Cod_Tipo_Rol";
                ...
                // ASIGNA CONTROL Y EVENTO AL BOTON DE MÁS ACCIONES
                this.btnAcciones = ((Button)this.Master.FindControl("Ribbon1").FindControl("TabContainer1").FindControl("tabAcciones").FindControl("cmdAccionesMas"));
                this.btnCopiarRol = ((Button)this.Master.FindControl("Ribbon1").FindControl("TabContainer1").FindControl("tabAcciones").FindControl("cmdCopiarRol"));
                this.btnCopiarRol.Click += new EventHandler(btnCopiarRol_Click);

                #endregion

                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoRol" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoRol", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodTipoRol", string.Empty, "Código", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoRol", string.Empty, "Descripción", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Código", "Cod_Tipo_Rol"));
        this.ddlFiltro.Items.Add(new ListItem("Descripción", "Des_Tipo_Rol"));
    }
    ...
    protected void btnBuscar_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                ...
                {
                    this.InformarBox1_SetConfirmationBoxEvent(sender, e, EnumTipoMensaje.Filtrado.ToString());
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        ...
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoRol");
    ...
    protected void btnCopiarRol_Click(object sender, EventArgs e)
    {
        try
        {
            RespuestaConsultaSesion sesion = wsSesiones.ConsultarSesion(idSesionOculto.Value);
            if (sesion.Codigo == 0)
            {
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() == 1) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            CheckBox checkBoxColumn = (CheckBox)gridView.Rows[row.RowIndex].FindControl("chkBox1");
                            if (checkBoxColumn.Checked)
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoRol");
                                pantallaIdOculto.Value = "0";
                                pantallaIdRoleOculto.Value = lbl.Text;
                                break;
                            }
                        }
                    ...
                    else//SI EXISTE MÁS DE UN REGISTRO SELECCIONADO O NINGUNO
                    {
                        this.InformarBox1_SetConfirmationBoxEvent(sender, e, "SYS_4");
                        this.mpeInformarBox.Show();
                    }
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        rolesEntidad.IdTipoRol = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(rolesEntidad);
                    }
                }
    ...
    private SeguridadWS.RespuestaEntidad Eliminar(TiposRolesEntidad _entidad)
    {
        return wsSeguridad.RolesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(SeguridadWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsSeguridad.RolesTotalFilas(_entidad);
    }

    private List<TiposRolesEntidad> Consulta(SeguridadWS.ParametrosConsultaEntidad _entidad)
    {
        return wsSeguridad.RolesConsultar(_entidad).ToList();
    }
    ...
    protected Dictionary<string, string> SetRutaVentana()
    {
        ...
        data.Add("idRole", pantallaIdRoleOculto.Value);
    ...
    private void BindGridView(GridView _gridView, List<TiposRolesEntidad> _lista)
    {
    ...
    private void MensajeUsuariosRoles(Label _mensaje)
    {
        try
        {
            Label mensaje = (Label)this.EliminarBox1.FindControl("lblMensajeEliminar");

            rolesEntidad.IdTipoRol = int.Parse(_mensaje.Text);

            int usuarios = wsSeguridad.RolesUsuariosConsultar(rolesEntidad);
            mensaje.Text = String.Format(Resources.ErroresBD.ROLEUSERS, usuarios);
        }
        catch (Exception ex)
        {
            throw ex;
