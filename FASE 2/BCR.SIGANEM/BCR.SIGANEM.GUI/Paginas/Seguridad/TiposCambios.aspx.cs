...
public partial class TiposCambios : System.Web.UI.Page
{
    ...
    private SeguridadWS.TiposCambiosEntidad tcEntidad = new SeguridadWS.TiposCambiosEntidad();
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdTipoCambio" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdTipoCambio", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesFecha", string.Empty, "Fecha", HorizontalAlign.Center, false, true));
        ...
        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("PorcentajeDevaluacion", string.Empty, "Porcentaje Devaluación", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Fecha", "Fecha"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdTipoCambio");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked == true)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        tcEntidad.IdTipoCambio = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(tcEntidad);
                    }
                }
    ...
    private SeguridadWS.RespuestaEntidad Eliminar(TiposCambiosEntidad _entidad)
    {
    ...
    {
        return wsSeguridad.TiposCambiosTotalFilas(_entidad);
    }

    private List<TiposCambiosEntidad> Consulta(SeguridadWS.ParametrosConsultaEntidad _entidad)
    {
        return wsSeguridad.TiposCambiosConsultar(_entidad).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<SeguridadWS.TiposCambiosEntidad> _lista)
    {
