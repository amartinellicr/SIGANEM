...
public partial class IndicesPreciosConsumidor : System.Web.UI.Page
{
    ...
    private SeguridadWS.IndicesPreciosConsumidorEntidad ipcEntidad = new SeguridadWS.IndicesPreciosConsumidorEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Ano";
                    consultaEntidad.ColumnaOrdenar = "Ano";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdIndicePrecioConsumidor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdIndicePrecioConsumidor", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Ano", string.Empty, "Año", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesMes", string.Empty, "Mes", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("Valor", string.Empty, "Valor", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("PorcentajeInflacion", string.Empty, "Porcentaje Inflación", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Año", "Ano"));
        this.ddlFiltro.Items.Add(new ListItem("Mes", "Des_Mes"));
    }
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdIndicePrecioConsumidor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked == true)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        ipcEntidad.IdIndicePrecioConsumidor = int.Parse(lbl.Text);

                        #endregion

                        resultado = this.Eliminar(ipcEntidad);
                    }
                }
                #region CONFIRMACIÓN ELIMINAR

                this.mpeInformarBox.Show();
                int value = resultado.ValorEstado;
                int valueError = resultado.ValorError;
    ...
    private SeguridadWS.RespuestaEntidad Eliminar(IndicesPreciosConsumidorEntidad _entidad)
    {
        //return wsConsultas.ActivosEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
        return null;
    }

    private Int32 TotalFilas(SeguridadWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsSeguridad.IndicesPreciosConsumidorTotalFilas(_entidad);
    }

    private List<SeguridadWS.IndicesPreciosConsumidorEntidad> Consulta(SeguridadWS.ParametrosConsultaEntidad _entidad)
    {
        return wsSeguridad.IndicesPreciosConsumidorConsultar(_entidad).ToList();
    }
    ...
    private bool AccesoPermisoPagina()
    {
        ...
        try
        {
            if (!Page.IsPostBack)
            {
                ...
                ((wucMenuSuperior)this.Master.FindControl("Ribbon1")).DeshabilitarBotonesNuevoModificarEliminar(true);
    ...
    private void BindGridView(GridView _gridView, List<SeguridadWS.IndicesPreciosConsumidorEntidad> _lista)
    {
