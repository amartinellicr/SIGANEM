...
public partial class InscripcionReales : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Id_Garantia_Real_Inscripcion";
                    consultaEntidad.ColumnaOrdenar = "Id_Garantia_Real_Inscripcion";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdInscripcionGarantiaReal" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdInscripcionGarantiaReal", lblID);
        ...
        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesIndInscripcion", string.Empty, "Ind. Inscripción", HorizontalAlign.Center, false, true));
    }
    ...
    private void CargarDDLFiltro()
    {
        ...
        this.ddlFiltro.Items.Add(new ListItem("Ind. Inscripción", "Des_Tipo_Indicador_Inscripcion"));  
    }
        ...
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    post.RedirectAndPOSTNewBigWindow(this.Page, "../Detalles/InscripcionRealesNew.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdInscripcionGarantiaReal");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        InscripcionGarantiasRealesEntidad.IdInscripcionGarantiaReal = int.Parse(lbl.Text);

                        //REQUERIMIENTO: 1-24381561
                        InscripcionGarantiasRealesEntidad.CodUsuarioIngreso = codUsuarioOculto.Value;

                        #endregion

                        resultado = this.Eliminar(InscripcionGarantiasRealesEntidad);
                    }
                }
    ...
    private GarantiasWS.RespuestaEntidad Eliminar(GarantiasWS.InscripcionGarantiasRealesEntidad _entidad)
    {
        return wsGarantias.InscripcionGarantiasRealesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(GarantiasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsGarantias.InscripcionGarantiasRealesTotalFilas(_entidad);
    }

    private List<InscripcionGarantiasRealesEntidad> Consulta(GarantiasWS.ParametrosConsultaEntidad _entidad)
    {
        return (wsGarantias.InscripcionGarantiasRealesConsultar(_entidad)).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<InscripcionGarantiasRealesEntidad> _lista)
    {
