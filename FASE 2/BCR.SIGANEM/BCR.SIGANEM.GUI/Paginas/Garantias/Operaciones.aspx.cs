...
public partial class Operaciones : System.Web.UI.Page
{
    ...
    private GarantiasWS.GarantiasOperacionesEntidad operacionesEntidad = new GarantiasWS.GarantiasOperacionesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Numero_Operacion";
                    consultaEntidad.ColumnaOrdenar = "Numero_Operacion";
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        _gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGarantiaOperacion", lblID); 

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoOperacion", string.Empty, "Tipo Operación", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesNumeroOperacion", string.Empty, "Número Operación", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("TipoIdentificacionRUC", string.Empty, "Tipo Identificación RUC", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("IdentificacionClienteRUC", string.Empty, "Identificación RUC", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Tipo Operación", "Des_Tipo_Operacion"));
        this.ddlFiltro.Items.Add(new ListItem("Número Operación", "Numero_Operacion"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Identificación RUC", "Des_Tipo_Identificacion_RUC"));
        this.ddlFiltro.Items.Add(new ListItem("Identificación RUC", "Identificacion_RUC")); 
    }
        ...
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    post.RedirectAndPOSTNewBigWindow(this.Page, "../Detalles/OperacionesNew.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGarantiaOperacion");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        operacionesEntidad.IdGarantiaOperacion = int.Parse(lbl.Text);
                        operacionesEntidad.CodUsuarioIngreso = codUsuarioOculto.Value;

                        #endregion

                        resultado = this.Eliminar(operacionesEntidad);
                    }
                }
                ...
                else
                {
                    switch (valorError)
                    {
                        ...
                        case 547:
                            ...
                            break;
                        case 13:
                            this.InformarBox1_SetConfirmationBoxEvent(sender, e, "SYS_13");
                            break;
                    }
                }
    ...
    private GarantiasWS.RespuestaEntidad Eliminar(GarantiasWS.GarantiasOperacionesEntidad _entidad)
    {
        return wsGarantias.GarantiasOperacionesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(GarantiasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsGarantias.GarantiasOperacionesTotalFilas(_entidad);
    }

    private List<GarantiasOperacionesEntidad> Consulta(GarantiasWS.ParametrosConsultaEntidad _entidad) 
    {
        return (wsGarantias.GarantiasOperacionesConsultar(_entidad)).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<GarantiasOperacionesEntidad> _dataSet)
    {
