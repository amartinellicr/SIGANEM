...
public partial class Fiduciarias : System.Web.UI.Page
{
    ...
    private GarantiasWS.GarantiasFiduciariasEntidad fiduciariasEntidad = new GarantiasWS.GarantiasFiduciariasEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Garantia";
                    consultaEntidad.ColumnaOrdenar = "Cod_Garantia";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdGarantiaFiduciaria" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGarantiaFiduciaria", lblID); 

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoIdentificacionRUC", string.Empty, "Tipo Identificación RUC", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodGarantia", string.Empty, "Id Fiador RUC", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("NombreRUC", string.Empty, "Nombre RUC", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoAvalFianza", string.Empty, "Tipo Aval o Fianza", HorizontalAlign.Center, false, true));
    }
    ...
    private void CargarDDLFiltro()
    {
        ...
        this.ddlFiltro.Items.Add(new ListItem("Id Fiador RUC", "Cod_Garantia")); 
        this.ddlFiltro.Items.Add(new ListItem("Nombre RUC", "Nombre_RUC")); 
        this.ddlFiltro.Items.Add(new ListItem("Tipo Aval o Fianza", "Des_Tipo_Aval_Fianza")); 
    }
        ...
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    post.RedirectAndPOSTNewWindow(this.Page, "../Detalles/FiduciariasNew.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGarantiaFiduciaria");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        fiduciariasEntidad.IdGarantiaFiduciaria = int.Parse(lbl.Text);
                        //REQUERIMIENTO: 1-24381561
                        fiduciariasEntidad.CodUsuarioIngreso = codUsuarioOculto.Value;

                        #endregion

                        resultado = this.Eliminar(fiduciariasEntidad);
                    }
                }
    ...
    private GarantiasWS.RespuestaEntidad Eliminar(GarantiasWS.GarantiasFiduciariasEntidad _entidad)
    {
        return wsGarantias.GarantiasFiduciariasEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(GarantiasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsGarantias.GarantiasFiduciariasTotalFilas(_entidad);
    }

    private List<GarantiasFiduciariasEntidad>Consulta(GarantiasWS.ParametrosConsultaEntidad _entidad) 
    {
        return (wsGarantias.GarantiasFiduciariasConsultar(_entidad)).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<GarantiasFiduciariasEntidad> _dataSet)
    {
