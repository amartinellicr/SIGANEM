...
public partial class MobiliariaReales : System.Web.UI.Page
{
    ...
    private GarantiasWS.MobiliariaGarantiasRealesEntidad MobiliariaGarantiasRealesEntidad = new GarantiasWS.MobiliariaGarantiasRealesEntidad();
    //private GarantiasWS.InscripcionGarantiasRealesEntidad InscripcionGarantiasRealesEntidad = new GarantiasWS.InscripcionGarantiasRealesEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Id_Garantia_Real_Mobiliaria";
                    consultaEntidad.ColumnaOrdenar = "Id_Garantia_Real_Mobiliaria";

                    #endregion

                    // BINDEA EL GRIDVIEW
                    this.BindGridView(gridView, this.Consulta(consultaEntidad)); //VALIDAR WS
                }
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdMobiliariaGarantiaReal" };
        ...
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    post.RedirectAndPOSTNewBigWindow(this.Page, "../Detalles/MobiliariaRealesNew.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdMobiliariaGarantiaReal");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        MobiliariaGarantiasRealesEntidad.IdMobiliariaGarantiaReal = int.Parse(lbl.Text);

                        //REQUERIMIENTO: 1-24381561
                        MobiliariaGarantiasRealesEntidad.CodUsuarioIngreso = codUsuarioOculto.Value;

                        #endregion

                        resultado = this.Eliminar(MobiliariaGarantiasRealesEntidad);
                    }
                }
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdMobiliariaGarantiaReal", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesNumeroOperacion", string.Empty, "Operación", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("NumeroBien", string.Empty, "Número Bien", HorizontalAlign.Center, false, true));
        ...
        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("FechaPublicacion", "{0:d}", "Fecha Publicación", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Operación", "Numero_Operacion"));
        ...
        this.ddlFiltro.Items.Add(new ListItem("Fecha Publicación", "Fecha_Publicacion"));
    }
    ...
    private GarantiasWS.RespuestaEntidad Eliminar(GarantiasWS.MobiliariaGarantiasRealesEntidad _entidad)
    {
        return wsGarantias.MobiliariaGarantiasRealesEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }

    private Int32 TotalFilas(GarantiasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsGarantias.MobiliariaGarantiasRealesTotalFilas(_entidad);
    }

    private List<MobiliariaGarantiasRealesEntidad> Consulta(GarantiasWS.ParametrosConsultaEntidad _entidad)
    {
        return (wsGarantias.MobiliariaGarantiasRealesConsultar(_entidad)).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<MobiliariaGarantiasRealesEntidad> _lista)
    {
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!disponible)
        {
            ...
            //inscripcionGarantiasRealesEntidad = null;
