...
public partial class Valores : System.Web.UI.Page
{
    ...
    private GarantiasWS.GarantiasValoresEntidad garantiasValoresEntidad = new GarantiasWS.GarantiasValoresEntidad();
        ...
        try
        {
            ...
            if (AccesoPermisoPagina())
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    consultaEntidad.ColumnaFiltro = "Cod_Garantia_BCR";
                    consultaEntidad.ColumnaOrdenar = "Cod_Garantia_BCR";
                ...
                // ASIGNA DATA KEYS
                String[] dataKeysString = { "IdGarantiaValor" };
    ...
    private void gridView_Init(object sender, EventArgs e)
    {
        ...
        gvTemplate.CrearCamposGridNoVisibles(gridView, "IdGarantiaValor", lblID);

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("CodGarantiaBCR", string.Empty, "Id Garantia BCR", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesInstrumento", string.Empty, "Identificación Instrumento", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesEmisor", string.Empty, "Identificación Emisor", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("MontoValorFacial", "{0:N2}", "Monto Valor Facial", HorizontalAlign.Center, false, true));

        this.gridViewColumna = new GridViewColumn();
        this.gridView.Columns.Add(this.gridViewColumna.CreateBoundField("DesTipoValor", string.Empty, "Tipo Valor", HorizontalAlign.Center, false, true));
    }

    private void CargarDDLFiltro()
    {
        // CARGA EL DDL FILTRO CON LOS FILTROS CORRESPONDIENTES A CADA MANTENIMEINTO
        this.ddlFiltro.Items.Add(new ListItem("Id Garantia BCR", "Cod_Garantia_BCR"));
        this.ddlFiltro.Items.Add(new ListItem("Identificación Instrumento", "Des_Instrumento"));
        this.ddlFiltro.Items.Add(new ListItem("Identificación Emisor", "Des_Emisor"));
        this.ddlFiltro.Items.Add(new ListItem("Monto Valor Facial", "Monto_Valor_Facial"));
        this.ddlFiltro.Items.Add(new ListItem("Tipo Emisor", "Des_Tipo_Valor"));
    }
        ...
        try
        {
            ...
            if (sesion.Codigo == 0)
            {
                if (Page.IsPostBack)
                {
                    ...
                    post.RedirectAndPOSTNewWindow(this.Page, "../Detalles/ValoresNew.aspx", SetRutaVentana());
                }
            }
        }
        catch (Exception ex)
                ...
                if (Page.IsPostBack)
                {
                    if (ContadorSeleccionados() < 2) //SI SOLO EXISTE UN REGISTRO SELECCIONADO
                    {
                        foreach (GridViewRow row in gridView.Rows)
                        {
                            ...
                            {
                                Label lbl = (Label)gridView.Rows[row.RowIndex].FindControl("lblIdGarantiaValor");
            ...
            if (sesion.Codigo == 0)
            {
                ...
                foreach (GridViewRow row in gridView.Rows)
                {
                    ...
                    if (checkBoxColumn.Checked)
                    {
                        ...
                        pantallaIdOculto.Value = lbl.Text;
                        garantiasValoresEntidad.IdGarantiaValor = int.Parse(lbl.Text);
                        //REQUERIMIENTO: 1-24381561
                        garantiasValoresEntidad.CodUsuarioIngreso = codUsuarioOculto.Value;

                        #endregion

                        resultado = this.Eliminar(garantiasValoresEntidad);
                    }
                }
    ...
    private GarantiasWS.RespuestaEntidad Eliminar(GarantiasWS.GarantiasValoresEntidad _entidad)
    {
        return wsGarantias.GarantiasValoresEliminar(_entidad, AsignarValoresBitacora(EnumTipoBitacora.ELIMINAR));
    }
    
    private Int32 TotalFilas(GarantiasWS.ParametrosTotalFilasEntidad _entidad)
    {
        return wsGarantias.GarantiasValoresTotalFilas(_entidad);
    }
    
    private List<GarantiasValoresEntidad> Consulta(GarantiasWS.ParametrosConsultaEntidad _entidad)
    {
        return (wsGarantias.GarantiasValoresConsultar(_entidad)).ToList();
    }
    ...
    private void BindGridView(GridView _gridView, List<GarantiasValoresEntidad> _lista)
    {
