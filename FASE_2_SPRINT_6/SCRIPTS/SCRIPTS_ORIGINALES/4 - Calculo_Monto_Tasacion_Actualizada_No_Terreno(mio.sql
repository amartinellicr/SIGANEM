...
ALTER PROCEDURE [dbo].[Calculo_Monto_Tasacion_Actualizada_No_Terreno]
AS 
/******************************************************************************************************************************************************
...
BEGIN
	...
	SELECT DISTINCT
			...
			Periodos = 0,
			Monto_Ultima_Tasacion_No_Terreno =	ISNULL(GR.Monto_Ultima_Tasacion_No_Terreno,0),
			Monto_Tasacion_Actualizada_No_Terreno = CAST((CASE	WHEN GR.Monto_Tasacion_Actualizada_No_Terreno IS NULL THEN ISNULL(GR.Monto_Ultima_Tasacion_No_Terreno,0)
																WHEN Fecha_Ultima_Tasacion_Garantia IS NULL THEN ISNULL(GR.Monto_Tasacion_Actualizada_No_Terreno,0)
																ELSE ISNULL(GR.Monto_Tasacion_Actualizada_No_Terreno,0)
													END) AS DECIMAL(22,2)),
			Monto_Depreciacion = CAST(	(CASE	WHEN GR.Monto_Ultima_Tasacion_No_Terreno IS NULL THEN 0
												WHEN Fecha_Ultima_Tasacion_Garantia IS NULL THEN ISNULL(GR.Monto_Tasacion_Actualizada_No_Terreno,0)
												ELSE ISNULL(GR.Monto_Ultima_Tasacion_No_Terreno,0)
										 END) AS DECIMAL(22,2)),
			Ciclo	= 0,
			...
			Ind_Nueva = CASE	WHEN AUX.Fecha_Calculo IS NULL THEN 'S' 
								WHEN AUX.Fecha_Calculo IS NOT NULL AND ((AUX.Monto_Ultima_Tasacion_No_Terreno < GR.Monto_Ultima_Tasacion_No_Terreno) OR (AUX.Monto_Ultima_Tasacion_No_Terreno > GR.Monto_Ultima_Tasacion_No_Terreno)) THEN 'M'
								ELSE 'N' END,
	...
	SELECT DISTINCT
			...
			Periodos = 0,
			Monto_Ultima_Tasacion_No_Terreno	= ISNULL(GR.Monto_Ultima_Tasacion_No_Terreno,0),
			Monto_Tasacion_Actualizada_No_Terreno	= CAST(	(CASE	WHEN GR.Monto_Ultima_Tasacion_No_Terreno IS NULL THEN 0
																	WHEN Fecha_Fabricacion_Garantia IS NULL THEN ISNULL(GR.Monto_Tasacion_Actualizada_No_Terreno,0)
															ELSE ISNULL(GR.Monto_Ultima_Tasacion_No_Terreno,0)
														END) AS DECIMAL(22,2)),
			Monto_Depreciacion = CAST(	(CASE	WHEN GR.Monto_Ultima_Tasacion_No_Terreno IS NULL THEN 0
																	WHEN Fecha_Ultima_Tasacion_Garantia IS NULL THEN ISNULL(GR.Monto_Tasacion_Actualizada_No_Terreno,0)
	...
	/*SE ELIMINAN LOS REGISTROS QUE HAN SIDO MODIFICADOS, ESTO PARA SER TRATADOS COMO SI FUERAN NUEVOS*/
	DELETE FROM dbo.AUX_ACTUALIZAGARANTIAS
	FROM dbo.GARANTIAS_REALES GR 
	INNER JOIN dbo.TIPOS_BIENES TB 
		ON GR.Id_Tipo_Bien = TB.Id_Tipo_Bien 
	INNER JOIN dbo.AUX_ACTUALIZAGARANTIAS AUX
		ON ISNULL(AUX.Id_Clase_Aeronave,-1) = ISNULL(GR.Id_Clase_Aeronave,-1)
		AND ISNULL(AUX.Id_Clase_Buque,-1) = ISNULL(GR.Id_Clase_Buque,-1)
		AND ISNULL(AUX.Id_Clase_Tipo_Bien,-1) = ISNULL(GR.Id_Clase_Tipo_Bien,-1)
		AND ISNULL(AUX.Id_Clase_Vehiculo,-1) = ISNULL(GR.Id_Clase_Vehiculo,-1)
		...
		AND ISNULL(AUX.Fecha_Ultima_Tasacion,'1900-01-01') = ISNULL(GR.Fecha_Ultima_Tasacion_Garantia,'1900-01-01')
		AND AUX.Ind_Bien = 'N'
	INNER JOIN dbo.AUX_GAR_MTO_ACT_NO_TERRENO AU1
		ON AU1.Id_Garantia_Real = GR.Id_Garantia_Real
	WHERE AU1.Ind_Nueva = 'M'
	...
	BEGIN 	
		...
		SET			Monto_Depreciacion = CASE Ind_Nueva WHEN 'S' THEN Monto_Ultima_Tasacion_No_Terreno
														WHEN 'M' THEN Monto_Ultima_Tasacion_No_Terreno
	...
	SET			GR.Monto_Tasacion_Actualizada_No_Terreno = CASE Ind_Nueva WHEN 'S' THEN ACT.Monto_Ultima_Tasacion_No_Terreno
																		  WHEN 'M' THEN ACT.Monto_Ultima_Tasacion_No_Terreno
	...
	SELECT 
		...
		Monto_Actualizado = CASE Ind_Nueva WHEN 'S' THEN AUX.Monto_Ultima_Tasacion_No_Terreno
										   WHEN 'M' THEN AUX.Monto_Ultima_Tasacion_No_Terreno
										   WHEN 'N' THEN AUX.Monto_Actualizado
							END,
		Ind_Bien = 'N',
		GR.Monto_Ultima_Tasacion_No_Terreno
	FROM dbo.GARANTIAS_REALES GR
	INNER JOIN dbo.AUX_GAR_MTO_ACT_NO_TERRENO AUX
